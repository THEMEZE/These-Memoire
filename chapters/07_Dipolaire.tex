...
Ici pour donner une visons simplement, on vas considerer un atomes à 2 niveaux.

\medskip

Considérons un atome modélisé par deux états, le fondamental $\ket{g}$ d'énergie $\hbar \omega_g$  et l’excité $\ket{e}$ d'énergie $\hbar \omega_e$, couplés par le champ électrique $\operatorvec{E}_\perp(\vec{r} , t ) = \operatorvec{\mathcal{E}}(\vec{r}) \cos ( \omega t )  $. le champ laser est loin de la résonance avec l’état excité $\ket{e}$ , c’est-à-dire que le désaccord $\Delta = \omega - \omega_{\scriptscriptstyle g\rightarrow e}$ (avec la fréquence de transition $\omega_{\scriptscriptstyle g\rightarrow e} = \omega_e - \omega_g $ ) est grand comparé à la fréquence de Rabi définissant le couplage entre le dipôle et le champ 
\begin{eqnarray}
	\Omega(\vec{r}) & = & -  \bra{e} \operatorvec{D} \cdot \operatorvec{\mathcal{E}}(\vec{r}) \ket{g}/\hbar \label{chap7:eq.Omega.1}\\
	& = &- \vec{d}_{\scriptscriptstyle g\leftrightarrow e } \cdot  \vec{\mathcal{E}}(\vec{r}) /\hbar\label{chap7:eq.Omega.2},	
\end{eqnarray}


avec $\vec{d}_{\scriptscriptstyle g\leftrightarrow e } = \bra{e} \operatorvec{D} \ket{g} $ l’élément de matrice dipolaire réel entre $\ket{g}$ et $\ket{e}$. 
...
\(
I_{\mathrm{sat}} = \frac{\hbar \, \omega_{\scriptscriptstyle e \rightarrow g}^3 \, \Gamma}{12 \pi c^2},
\) 
de la largeur naturelle \eqref{chap7:eq.Gamma.1} et de la fréquence de Rabi \eqref{chap7:eq.Omega.2}, le potentiel dipolaire \eqref{chap7:eq.dip.1} s'écrit sous la forme opérationnelle du potentiel dipolaire devient :
\begin{equation}
