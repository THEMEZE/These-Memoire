...
{\color{blue} Non, tu ne fais pas ça. Donc enlève ça.}
...
{\color{blue}Je trouve bizarre que tu te places dans un contexte si large.}
...
\paragraph{Paramétrisation spectrale et densité conservée}
          {\color{blue} Tu as déjà introduit $\theta$ et $\rho$ dans les chapitres précédents ! Donc ici, cela fait très bizarre. On sent que tu as fait du copier-collé de l'intro d'un article théorique. Je propose la formulation suivante.
 ``Dans le cas du modèle de Lieb-Liniger, on a vu que l'ensemble
            des quantités locales conservées (voir xxx pour la définition d'une quantité locale conservée ( cette expression étant un abus de langage (à dire dans xxx, pas ici))) peuvent être représentées par la distribution de rapidité $\rho(\theta)$. Dans l'approche hydrodynamique, $\rho$ dépend de la position $x$ et devient donc une fonction de deux paramètres  $\rho(x,\theta)$. On appelle espace spectral l'espace $(x,\theta)$. Dans cet espace, $\rho$ joue le rôle de densité.      }      

          La GHD reformule l’infinité de lois de conservation (éventuellement rompues) en une famille indexée par un paramètre spectral continu $\theta$, plutôt que par un indice discret $i$. On note $\rho(x,\theta,t)$ la densité conservée en espace réel, espace spectral et temps. Le paramètre spectral énumère les objets asymptotiques issus de la théorie de diffusion correspondante (particules, solitons, etc.), incluant leur quantité de mouvement et leurs éventuels degrés internes. Dans de nombreux cas simples, $\theta$ appartient à un sous-ensemble de $\mathbb{R}$, représentant les moments {\color{blue} pas moment: quantités de mouvemenet, ou impulsion. Ici on sent un peu beaucoup google translate !} asymptotiques, et les coordonnées $(x,\theta)$ forment un « espace des phases spectral » sur lequel $\rho$ joue le rôle de densité.
...
{\color{blue}
  ...
  Le point technique ardu de la GHD est la dérivation de  la vitesse effective et l'accélération effective apparaissant dans \eqref{chap:GHD:eq.GHD.1}.
Ces quantités ont été calculées 
pour la première fois par Bertini et al. (2016) et Castro-Alvaredo et al. (2016). Cette observation clé a déclenché l’ensemble des développements ultérieurs de la dynamique hydrodynamique généralisée (GHD) dans les systèmes quantiques intégrables. Les travaux de Bertini et al. (2016) s’appuient en partie sur ceux de Bonnes et al. (2014), où la formule donnant la vitesse effective \eqref{chap:GHD:eq.nu.v.a.1} était apparue pour la première fois dans le contexte d’un système quantique intégrable.

  Les vitesses et accélérations effectives s'écrivent
  \begin{equation}%\label{chap:GHD:eq.nu.v.a.1}
    veff
    aeff
	\quad 2 \pi \, v^{\mathrm{eff}} \, \rho  =(Id )^{\mathrm{dr}}_{[\nu]} \, \nu , 
	\quad 2 \pi \, a^{\mathrm{eff}} \, \rho  = -(\partial_x V )^{\mathrm{dr}}_{[\nu]}\, \nu ,
\end{equation}
où $Id$ est la fonction $\theta \rightarrow \theta$ et l'habillage, noté $^{\rm dr}$, est défini au chapitre xx, Eq. xx. 
...
\paragraph{Dérivation intuitive de la vitesse effective}
Ici tu remets ton paragraphe
}
...
{\color{blue} Pas si évident, et se dérive différemment si $X=x$ et $X=\theta$.}
\medskip
...
{\color{blue} le $\nu$ devant le $\Delta$ ne doit pas être là il me semble. Aussi un $\nu$ est écrit $v$.}
\begin{eqnarray}
