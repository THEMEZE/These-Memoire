...
\begin{eqnarray}
    ...
    v^{\mathrm{eff}}(\theta_-) &=& \tfrac{3}{4}\theta_- + \tfrac{1}{4}\theta_+. \label{eq:veff-}
\end{eqnarray}
En introduisant la densité $n$ \,  ($4\sqrt{gn} = \theta_+ - \theta_-$ ) et la vitesse du fluide $u$ définies plus haut les équations~\eqref{eq:theta+}--\eqref{eq:theta-} se réécrivent sous la forme :  
\begin{eqnarray}
    \partial_t n + \partial_x \big( u n \big) &=& 0, \label{eq:GHDcontinuite}\\
    \partial_t u + u \partial_x u &=& - g\, \partial_x n. \label{eq:GHDeuler}
\end{eqnarray}
...
\begin{itemize}[label = $\star$]
    ...
    \item Dans le cas d'un système chaotique (non intégrable), cet état localement relaxé est bien décrit par un ensemble de Gibbs classique, paramétré par la densité locale $\langle \hat{n} \rangle$ et l'énergie par particule $\langle \hat{e} \rangle$.
    ...
    \item Dans la limite de l'état fondamental et pour un paramètre de Lieb-Liniger $\gamma \to 0$, les équations GHD se réduisent aux équations hydrodynamiques obtenues à partir des équations de Gross-Pitaevskii dépendantes du temps, lorsque le terme de pression quantique est négligeable.
\end{itemize}

Ainsi, la GHD fournit un cadre rigoureux pour décrire la dynamique hors équilibre des systèmes intégrables à grande échelle, reliant la description microscopique des rapidités à des équations hydrodynamiques efficaces.
