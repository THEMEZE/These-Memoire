...
\begin{eqnarray}\label{chap:3:eq:hydro.1}
	...
	\partial_t  u + \partial_x (u^2/2 + \mathcal{P}/n) &=& -\, \partial_x V(x), \\
...
On a vus l'équation \gls*{GP} dépendant du temps dans \eqref{chap.1:eq.GP.2}, avec $\hbar = m = 1$.En utilisant la représentation de Madelung $\psi(x,t) = \sqrt{n(x,t)}\, e^{i\theta(x,t)}$, on obtient les équations dites hydrodynamiques de la forme de \eqref{chap:3:eq:hydro.1} :  
\begin{eqnarray}
    \partial_t n + \partial_x \big( n u \big) &=& 0, \label{eq:continuite}\\
    \partial_t u + \partial_x \left ( \frac{u^2}{2} + \left ( \frac{\mathcal{P} + \mathcal{P}_Q}{n} \right )  \right ) &=& 0, \label{eq:euler}
\end{eqnarray}
où la vitesse du fluide , la pression , et la \emph{pression quantique} sont définies par
\begin{equation}\label{eq:vitesse}
    u =  \partial_x \theta, \quad \mathcal{P} = \frac{1}{2}qn^2 ,\quad \mathcal{P}_Q = - \frac{1}{2} \sqrt{n}\partial_x^2( \sqrt{n}).
\end{equation}


Dans la limite dite de \gls{TF} 1D, les fluctuations de densité sont faibles et le terme de pression quantique peut être négligé \cite{PhysRevA.58.2385}. On retrouve alors la relation d’état 
\(
    n g = \mu,
\)
