...
\subparagraph*{En l’absence de force externe,} l’opérateur nombre de rapidités
\begin{equation}
    \Pi(\theta)\,\delta\theta
    = \int \mathrm{d}x \, \rho(x,\theta;t)\,\delta\theta
\end{equation}
est conservé dans chaque tranche de rapidité
\([\theta,\theta+\delta\theta]\), et ce pour tout \(\theta\)
(cf. Fig.~\ref{fig:Piconserved}).

Dans la limite \(\delta\theta \to 0\), l’ensemble de ces lois de conservation se traduit par l’équation de continuité
\begin{equation}
    \partial_t \rho(x,\theta;t)
    + \partial_x\!\left(v^{\mathrm{eff}}(\theta)\,\rho(x,\theta;t)\right)
    = 0,
\end{equation}
qui correspond à l’équation de la théorie \gls{GHD} \eqref{chap:GHD:eq.GHD.1}, mais sans terme de dérivée en \(\theta\).

\begin{figure}[htbp]
    \centering
	\includegraphics[width=0.6\textwidth , page = 1 ]{figures/03_GHD/Figures.pdf}
    \caption{
    Conservation du nombre de quasi-particules dans une tranche de rapidité
    \([\theta,\theta+\delta\theta]\) en l’absence de force externe.
    }
    \label{fig:Piconserved}
\end{figure}

\subparagraph*{En présence d’une force externe,} les rapidités évoluent dans le temps et les quasi-particules peuvent traverser les tranches de rapidité \([\theta,\theta+\delta\theta]\).
La conservation locale est alors modifiée, et l’on obtient l’équation complète de la \gls{GHD} \eqref{chap:GHD:eq.GHD.1} .
...
\begin{figure}[htbp]
    ...
    %\includegraphics[width=0.8\textwidth,page=2]{figures/03_GHD/Figure.pdf}
...
\begin{figure}[h!]
    \centering
     \includegraphics[width=0.6\textwidth , page = 3 ]{figures/03_GHD/Figures.pdf}
