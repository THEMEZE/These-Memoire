...
Nous développons également une méthode expérimentale originale permettant de mesurer localement le facteur d’occupation $\nu(x,\theta)$ dans la région de déformation. La \gls*{GHD} prédit que cette quantité révèle une forte asymétrie : large et lisse du côté initialement occupé, abrupte et tronquée du côté initialement vide. Cette assymétrie reflète le fais que, localement, le système est dans un état non thermique puisque ces derniers ont un facteur d'occupation $\nu(\theta)$ symétrie.%la nature hors équilibre du système, et s’accorde qualitativement avec les prédictions de la GHD, bien que la résolution spatiale expérimentale limite l’accès aux détails les plus fins.
...
Une fois la coupure effectuée, le confinement longitudinal est supprimé tandis que le confinement transverse est maintenu. Le gaz évolue alors librement dans une direction, ce qui est analogue  à  \eqref{chap6:eq.Rieman.1} mais depuis une condition initiale de deux regions homogène (problème de Riemann) \eqref{chap6:eq.Rieman.cond.1} . L’évolution du profil de densité $n(x,t)$ est mesuré par imagerie après différentes durées $t$ d’évolution.
...
Les équation de transport pur \eqref{eq:nu.cont} ne sont pas  des cas particuliers de ~\eqref{chap6:eq.Rieman.1}. Et donc même si les conditions initials de type  « coupure bipartite » , pour $\nu$ :
\begin{equation}
...
\paragraph{Résolution de l'évolution de $\nu$.} L'équation \eqref{eq:nuvsnuetoile} montre que la dynamique est  entièrement encodée dans la fonction $\nu^*(\xi,\theta)$, qui décrit la structure locale du gaz le long des rayons de vitesse $\xi$. Pour différent  $\theta$ , \eqref{eq:nu.cont},  se réécrit  , en utilisant \eqref{eq:nuvsnuetoile},   avec un dérivé selon $\xi$ : 
\begin{equation}
...
\end{equation}
où la distribution de rapidité extensive est $\Pi(\theta) = \int \rho(x, \theta ; \tau \geq 0)\, dx $ est une constante du mouvement. Cette distribution peut etre calculée en utilisant l'état initial :
...
Le profils $\tilde{n}_{\text{pair}}^{(x_s)}(x)$ et $\tilde{n}_{\text{impair}}^{(x_s)}(x)$ obtenus pour cette valeur de $x_s$ sont montré figure \ref{fig:simul_deformation} (b). Un asymétrie est bien visible. Nous pouvons donc affirmer que le profil expérimental présente une asymétrie. 
...
$\Pi$ est l'integrale \eqref{chap6:eq.Pi.1}. Si $\rho(x,\theta)$ est uniforme sur la largeur $\ell$, $\Pi(\theta) \simeq \ell\, \rho(x_0,\theta) $ soit :
\begin{equation}
...
(e) À l'instant $t = 18~\mathrm{ms}$, après l'évolution balistique post-quench, le facteur d'occupation est donné par $\nu^\ast(x_b(t)/t, \theta) = \nu_0(\theta)$ pour $\theta < \theta_b$, et nul pour $\theta > \theta_b$, résolvant l'équation~(\ref{eq:nuetoile}), pour $t>0$. $\nu(x(t), \theta)(=\nu^\ast(x(t)/t, \theta))$ est invarient de la déformation ie de $t>0$.  Le bord représenté en tirets par l'ensemble des points $(x_b(t)/t,\, \theta( t))$. Étant donné que la coupure initiale est en $x = 0$ et que l’évolution du bord est balistique, cette courbe résoud $v^{\mbox{\tiny eff}}_{[\nu^\ast(x(t)/t, \cdot) ]}(\theta) = x(t)/t = v)$ (\ref{eq:nu.cont}).
...
\paragraph{Étapes calcules numérique.}
