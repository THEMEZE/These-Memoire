...
L’application de la \gls*{GHD} à l’échelle d’Euler dans notre expérience est limitée par des contraintes pratiques qui définissent une fenêtre temporelle $t \in [t_m, t_M]$ d’observation fiable : 

\begin{itemize}[label = $\bullet$]
  \item Pour $t > t_M \simeq 18$~ms, la densité atomique devient trop faible et les effets de taille finie compromettent l’approximation d’un système quasi-infini.
  \item Pour $t < t_m \simeq 6 \pm 2$~ms, plusieurs facteurs réduisent la pertinence de la comparaison avec la \gls*{GHD} :
...
Ainsi, dans nos conditions expérimentales, la comparaison avec la \gls*{GHD} est la plus pertinente pour des temps compris entre $t_m \approx 6$~ms et $t_M \approx 18$~ms.
...
Pour accéder expérimentalement à cette observable, nous nous appuyons sur un protocole développé dans notre équipe \cite{dubois_probing_2024} (présenté en détail en fin du Chapitre~5), qui permet de reconstruire indirectement la distribution en rapidité à partir de l’expansion libre d’une tranche localisée du gaz. En parallèle, nous comparons ces mesures aux simulations numériques issues de la GHD, afin de tester directement la présence de la discontinuité prédite.
...
\begin{itemize}[label=$\bullet$]
    \item le profil de densité du nuage juste avant l’impulsion du faisceau pousseur (courbe rose de la figure~\ref{fig:6.3}) ;
    \item le profil de densité après l’application du faisceau pousseur, après un délai incompressible de $1~\mathrm{ms}$ (courbe blue de la figure~\ref{fig:6.3}).
\end{itemize}
...
Nous considérons d’abord un gaz unidimensionnel homogène infini, à l’équilibre thermique, caractérisé par le couple de paramètres thermodynamiques $(T, \mu)$. Comme expliqué au chapitre~\ref{chap:...}, la thermodynamique de Bethe-Ansatz permet de déterminer la distribution de quasi-particules $\nu_0(\theta)$ et les densités associées $\rho_0(\theta)$ et $\rho_s(\theta)$ via le processus de {\em dressing}.  
...
(e) À l'instant $t = 18~\mathrm{ms}$, après l'évolution balistique post-quench, le facteur d'occupation est donné par $\nu^\ast(x_b(t)/t, \theta) = \nu_0(\theta)$ pour $\theta < \theta_b$, et nul pour $\theta > \theta_b$, résolvant l'équation~(\ref{eq:nuetoile}), pour $t>0$. $\nu(x(t), \theta)(=\nu^\ast(x(t)/t, \theta))$ est invarient de la déformation ie de $t>0$.  Le bord représenté en tirets par l'ensemble des points $(x_b(t)/t,\, \theta(s; t))$. Étant donné que la coupure initiale est en $x = 0$ et que l’évolution du bord est balistique, cette courbe résoud $v^{\mbox{\tiny eff}}_{[\nu^\ast(x(t)/t, \cdot) ]}(\theta) = x(t)/t = v(s)$ (\ref{eq:nu.cont}).
...
L’analyse repose sur la \gls*{GHD}, cadre théorique récent permettant de décrire, à l’échelle d’Euler, la dynamique de systèmes quantiques intégrables. Nous avons vus que la \gls*{GHD} permet non seulement de prédire les profils de densité issus de la dynamique balistique du gaz, mais également de modéliser la distribution locale des quasi-particules, accessibles via un protocole de sélection spatiale.
