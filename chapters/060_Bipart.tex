...
Le problème de Riemann correspond au cas où la situation initiale est composée de deux régions homogènes, juxtaposées de part et d’autre de l’origine :
...
Dans ce cadre, le problème de Riemann \eqref{chap6:eq.Rieman.cond.1} se généralise naturellement au cas d’un nombre infini de champs hydrodynamiques $u$, engendrant une dynamique gouvernée par une hiérarchie de lois de conservation.
...
À l’échelle d’Euler, le cadre de la \gls*{GHD} ne décrit pas l’état initial de type Riemann \eqref{chap6:eq.Rieman.cond.1}, mais devient valide après une courte évolution temporelle.  
Nous montrons que la discontinuité initiale engendre une propagation balistique, en accord avec la solution du problème de Riemann généralisé.  
...
Une fois la coupure effectuée, le confinement longitudinal est supprimé tandis que le confinement transverse est maintenu. Le gaz évolue alors librement dans une direction, ce qui est analogue  à  \eqref{chap6:eq.Rieman.1}, mais depuis une condition initiale de deux régions homogènes (problème de Riemann) \eqref{chap6:eq.Rieman.cond.1}. L’évolution du profil de densité $n(x,t)$ est mesuré par imagerie après différentes durées $t$ d’évolution.
...
Dans cette section, nous présentons les résultats expérimentaux obtenus après la coupure bipartite, et les comparons aux prédictions de la \gls*{GHD} à l’échelle d’Euler. L’objectif est de vérifier que la dynamique du gaz est bien décrite, à court et moyen terme, par une solution auto-similaire des équations hydrodynamiques.
...
L’application de la \gls*{GHD} à l’échelle d’Euler dans notre expérience est limitée par des contraintes pratiques qui définissent une fenêtre temporelle $t \in [t_m, t_{\mathrm{max}}]$ d’observation fiable : 
...
Nous avons étudié la dynamique hors équilibre d’un gaz unidimensionnel de bosons après une coupure bipartite, créant une discontinuité initiale dans le profil de densité. Cette configuration, analogue au problème de Riemann quantique, engendre une évolution balistique qui peut être décrite par la Théorie d’Hydrodynamique Généralisée (\gls*{GHD}).
...
\begin{itemize}[label=$\bullet$]
    ...
    \item le profil de densité après l’application du faisceau pousseur, après un délai incompressible de $1~\mathrm{ms}$ (courbe blue de la figure~\ref{fig:simul_deformation} a)).
\end{itemize}
...
\end{equation}
où la distribution de rapidités extensive est $\Pi(\theta) = \int \rho(x, \theta ; \tau \geq 0)\, dx $ est une constante du mouvement. Cette distribution peut etre calculée en utilisant l'état initial :
...
La figure \ref{fig:simul_deformation}(b) présente deux courbes : le profil mesuré $\tilde{n}(x)$ (abréviation de $\tilde{n}(x,\tau)$) et son image par symétrie par rapport à un axe $x = 2x_s$. L’objectif est de rendre visible l’asymétrie du profil.
...
Nous considérons d’abord un gaz unidimensionnel homogène infini, à l’équilibre thermique, caractérisé par le couple de paramètres thermodynamiques $(T, \mu)$. Comme expliqué au chapitre~\ref{chap:relaxation}, la thermodynamique de Bethe-Ansatz \gls{TBA} permet de déterminer la facteur d'occupation $\nu_0(\theta)$ et la distribution de rapidités  $\rho_0(\theta)$ et  densité d'état $\rho_s(\theta)$ via \eqref{eq:dressing}.  
...
Nous pouvons donc paramétrer le contour par \(\theta^*\). 
...
(e) À l'instant $t = 18~\mathrm{ms}$, après l'évolution balistique post-quench, le facteur d'occupation est donné par $\nu^\ast(x_b(t)/t, \theta) = \nu_0(\theta)$ pour $\theta < \theta_b$, et nul pour $\theta > \theta_b$, résolvant l'équation~(\ref{eq:nuetoile}), pour $t>0$. $\nu(x(t), \theta)(=\nu^\ast(x(t)/t, \theta))$ est invariant de la déformation pour $t>0$.  Le bord représenté en tirets par l'ensemble des points $(x_b(t)/t,\, \theta( t))$. Étant donné que la coupure initiale est en $x = 0$ et que l’évolution du bord est balistique, cette courbe résout $v^{\mbox{\tiny eff}}_{[\nu^\ast(x(t)/t, \cdot) ]}(\theta) = x(t)/t = v)$ (\ref{eq:nu.cont}).
...
La Fig.\ref{fig:simul_expansion}(a) montre le profil d’expansion simulé à partir d’un état initial à température $T = 560~nK$, déterminée indépendamment par ajustement sur la déformation du bord (cf. Fig.\ref{fig:simul_deformation}). Le profil présente une forte asymétrie caractéristique, avec un bord droit abrupt et une densité qui s'annule au-delà d’une certaine position. Toutefois, cette chute est moins abrupte que celle de la distribution locale des rapidités $\rho(x, \theta)$ au centre de la tranche (courbes $\tau n_{\mathrm{GHD}}$ et $\ell \rho$ de la figure\ref{fig:simul_expansion}(a)). Deux effets principaux expliquent cet élargissement :
\begin{itemize}
...
Une analyse plus fine du profil expérimental met en évidence, du côté des petites valeurs de $x$ (courbe bleue, Fig.~\ref{fig:simul_expansion}(b)), la présence de défauts absents des prédictions de la \gls*{GHD} à l’échelle d’Euler.  
...
\end{tikzpicture}
\caption{(a) {\it Profil de densité après expansion de la tranche : effets de la largeur finie et du temps d’expansion fini.} Courbe rouge : profil obtenu par simulation GHD après expansion pendant $\tau = 30$ ms, avec $T = 560$ nK. Courbe marron : distribution asymptotique $\Pi((x - x_0)/\tau)/\tau$. Courbe pointillée noire : approximation $\ell \rho(x_0, (x - x_0)/\tau)/\tau$ dans le cas d’une tranche étroite. (b) {\it Comparaison aux données expérimentales.} En bleu : profil expérimental après expansion pendant $\tau = 30$ ms. En rouge : simulation GHD avec $T = 560$ nK. En gris : ajustement du profil expérimental donnant $T = 1550$ nK.}
...
Les simulations reproduisent qualitativement les principaux traits observés expérimentalement, notamment l’asymétrie du profil d’expansion et la chute abrupte du bord. Cependant, des écarts significatifs subsistent, notamment dans les régions périphériques et dans la forme fine des profils. Ces différences soulignent les limites du modèle à l’échelle d’Euler, et suggèrent que des effets hors modèle — tels que  le chauffage local, ou des corrections diffusives — peuvent jouer un rôle non négligeable.

\medskip

Ces résultats confirment la pertinence de la \gls*{GHD} pour décrire la dynamique collective d’un gaz unidimensionnel hors équilibre, tout en ouvrant la voie à des extensions du modèle pour capturer des effets plus fins, au-delà de l’approximation hydrodynamique idéale.
...
L’analyse repose sur la \gls*{GHD}, cadre théorique récent permettant de décrire, à l’échelle d’Euler, la dynamique de systèmes quantiques intégrables. Nous avons vus que la \gls*{GHD} permet non seulement de prédire les profils de densité issus de la dynamique balistique du gaz, mais également de modéliser la distribution locale des rapidités, accessibles via un protocole de sélection spatiale.
