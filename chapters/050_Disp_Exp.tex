	...
	\begin{tikzpicture}
			\begin{scope}[shift={(2,0)}]
				...
				\begin{scope}[scale=1.0]
					\input{figures/05_Disp_Exp/Etat_hiperfine_code}	
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
			\begin{scope}[shift={(0,0)}]
				...
				%\begin{scope}[scale=0.5]
					\input{figures/05_Disp_Exp/Structure_fils}	
			...
			\begin{scope}[shift={(9,0)}]
				\begin{scope}[transform canvas={scale=0.6}]
				%\begin{scope}[scale=0.5]
					\input{figures/05_Disp_Exp/Tranche_puce}	
	...
	\end{tikzpicture}
	\caption{a) {\bf Zoom sur la zone d’intérêt}. Les fils {\color{\colorFour}U (bleu)} et {\color{\colorSix} Z (rouge vif)} créent des pièges magnétiques intermédiaires, les trois micro-fils {gris} forment le guide longitudinal, et les fils {\color{\colorFive} d/d' (marron)} et {\color{\colorThree} D/D' (gris)} assurent le confinement longitudinal. b) Schéma des différentes couches constituant la puce atomique.}% (Figs adaptée de \cite{L.Dubois2024})}
...
Une fois refroidi, le nuage est acheminé vers la zone expérimentale où trois micro-fils parallèles et symétriques (fils gris) parcourus par des courants alternatifs (\gls{AC}) génèrent un guide magnétique unidimensionnel assurant le confinement transverse. Le confinement longitudinal est fourni par deux paires de fils : $d/d'$ (marron) et $D/D'$ (gris).
...
\end{eqnarray*}
comme on le fait dans \cite{Dubois2025}.
...
Ce dispositif permet ainsi de générer des protocoles analogues à des configurations classiques comme le \emph{pendule de Newton}, ou de sonder directement la dynamique d’un gaz de \acrlong{LL} dans des conditions contrôlées. Il constitue une brique essentielle pour les expériences de dynamique et de transport quantique.


\subsection{Mise en place technique}
...
En sélectionnant une tranche du gaz, on peut ensuite couper le confinement longitudinal et laisser cette tranche s’étendre. Le profil de densité asymptotique obtenu après un long temps d’expansion est proportionnel à la distribution de rapidité locale du gaz initial. Ce protocole permet ainsi une mesure résolue en $x$ de \(t\, n(x,t \to \infty) \sim \tilde{n}(x/t, t \to \infty)\).

\paragraph{Génération d’états hors équilibre}

La sélection permet également de créer des discontinuités dans le profil de densité, et donc d’initier une dynamique hors équilibre. Par exemple, on peut ne conserver que deux paquets séparés de gaz, qui vont alors osciller l’un vers l’autre. Cette configuration est analogue à un \emph{pendule de Newton quantique} \cite{10.21468/SciPostPhys.6.6.070,Kinoshita}.
...
Le montage permet de couvrir une zone de l’ordre de \(600\,\mu\mathrm{m}\) dans le plan des atomes, soit plus que la longueur typique d’un nuage (\(\sim 400\,\mu\mathrm{m}\) pour \(\omega_{\parallel}= 2 \pi \times 5\,\mathrm{Hz}\)). Le grandissement est déterminé par les focales utilisées : une focale \(f_{\mathrm{DMD}} = 750\,\mathrm{mm}\) du côté du \gls*{DMD}, et \(f_{\mathrm{at}} = 32\,\mathrm{mm}\) pour l’objectif côté atomes, donnant \(G = f_{\mathrm{at}}/f_{\mathrm{DMD}} \approx 0.043\) (Fig ~ \ref{chap:5:fig:Shema_Glob} ).
...
\begin{itemize}[label = $\bullet$]
    ...
    \item Des franges d’interférences apparaissaient lors de la division des images d’absorption, probablement dues à des effets {\em Fabry-Pérot} dans les optiques. Le désaxage du faisceau d’imagerie a permis d’en limiter l’impact.
...
Concrètement, l’expansion longitudinale est réalisée selon la séquence :
\begin{itemize}[label = $\bullet$]
  \item Le nuage est initialement piégé dans un potentiel magnétique caractérisé par une fréquence longitudinale $\omega_{\parallel} = 2 \pi \times  5.0$ ou $2 \pi \times  9.4\,\mathrm{Hz}$ selon les jeux de données, et une fréquence transverse $\omega_{\perp} = 2 \pi \times  2.56\,\mathrm{kHz}$.
  \item À $t=0$, le confinement longitudinal est éteint en annulant les courants $I_D=I_{D'}=0$. La coupure est réalisée sur un temps fini $t_{\parallel} = 70\,\mu\mathrm{s} \ll 2 \pi /\omega_{\parallel}$, ce qui évite un pic de courant parasite.% tout en préservant la dynamique du gaz.
  \item Le nuage se dilate librement dans la direction longitudinale pendant une durée $\tau$. Ensuite, le confinement transverse est relâché en annulant $I_{\perp}$, avec un temps de coupure $t_{\perp} = 5\,\mu\mathrm{s} \ll 2 \pi /\omega_{\perp}$.
...
Pour un système dans les régimes \gls{qBEC} (pour un paramètre de Lieb définie en \ref{chap2:eq:gamma.1} , $\gamma  \ll 1$ et $T \sim 0$), la dynamique du système étudié est décrite par l’équation de \gls{GP} \eqref{chap.2:eq.GP.2} : 
\begin{eqnarray*}
	i \partial_\tau\phi = \left \{ - \frac{1}{2}\Delta_{\vec{r}} + V(\vec{r}) + g_{\mathrm{3D}} N \vert \phi \vert^2 \right \} \phi,
\end{eqnarray*}
avec $g_{\mathrm{3D}} = 4 \pi a_{\mathrm{3D}}$ et en présence d’un potentiel externe (voir \eqref{chap5:eq.pot.perp.1} et \eqref{chap5:eq.pot.long.1}) :
\begin{eqnarray*}
...
En utilisant la transformation de \emph{Madelung} 
\(
\psi(x,\tau) = \sqrt{n(x,\tau)} \, e^{i \vartheta(x,\tau)},
\)
et en introduisant la vitesse \(u = \partial_x \vartheta\), on obtient les équations hydrodynamiques associées \cite{Spiegel1980FluidDF,10.1093/acprof:oso/9780198758884.001.0001} :
\begin{eqnarray}\label{chap:5:eq.hydro.1}
	\left\{
	\begin{array}{rcl}
		\partial_\tau n + \partial_x ( n u )	 & = & 0, \\[0.3em]
		\partial_\tau u + \partial_x \left( \frac{u^2}{2} + V_\parallel + \mu(n) + \frac{\mathcal{P}_Q}{n} \right) & = & 0,
	\end{array} 
	\right.
\end{eqnarray}
...
\begin{eqnarray}\label{chap:5:eq.hydro.1.1}
	...
	\begin{array}{rcl}
		\partial_\tau n + \partial_x ( n u )	 & = & 0, \\[0.3em]
		\partial_\tau u + \partial_x \left( \frac{u^2}{2} + V_\parallel + \mu(n) \right) & = & 0,
	\end{array} 
	\right.
\end{eqnarray}
...
\begin{eqnarray}
	\mu(n(x)) + V_\parallel(x) = \mu\bigl(n(x=0)\bigr).
\end{eqnarray}
Pour $\tau \geq 0$, le potentiel longitudinal est éteint : $V_\parallel(x) = 0$.

\medskip

\paragraph{Solutions analytiques homothétiques.}
Si $n$ est solution des équations hydrodynamiques \eqref{chap:5:eq.hydro.1.1}, pour $\tau \geq 0$; On fait l'hypothèse que la densité linéaire suit une forme homothétique
\begin{eqnarray}\label{chap:5:eq.hydro.1.3}
	n(x,\tau) = \frac{1}{\lambda(\tau)} n_0 \left ( \frac{x}{\lambda(\tau)} \right ) ,	
\end{eqnarray}
avec $n_0$ le profil de densité à $\tau = 0 $ et $\lambda(\tau)$ le facteur d'échelle à un temps d'expansion $\tau$. Avec les contraintes $\lambda(0) = 1$, $\lambda'(0) = 0$ et $N = \int dx \, n(x , \tau ) $. En injectant dans \eqref{chap:5:eq.hydro.1} il vient que 
\begin{eqnarray}\label{chap:5:eq.hydro.2}
	...
	\right.
\end{eqnarray}
(car \(\partial_\tau u=(\ddot\lambda/\lambda-\dot\lambda^2/\lambda^2)x\) et \(u\partial_x u=(\dot\lambda/\lambda)^2 x\), leur somme donne \((\ddot\lambda/\lambda)x\)) et initialement% $\mu( n_0 ( x ) ) = \mu( n_0 ( x = 0  ) ) - \frac{1}{2} \omega_\parallel^2 x^2 $.
\begin{eqnarray}
...
En utilisant l'équilibre \(\mu\big(n_0(y)\big)=\mu_p-\tfrac12 \omega_\parallel^2 y^2\), soit \(\mu'(n_0(y))\,n_0'(y)=-\omega_\parallel^2 y\) soit 
\begin{eqnarray}\label{chap:5:eq.hydro.2.3}
	n_0'(y)=-\frac{\omega_\parallel^2\,y}{\mu'(n_0(y))}.	
\end{eqnarray}
En injectant \eqref{chap:5:eq.hydro.2.3} dans \eqref{chap:5:eq.hydro.2.2} et en injectant ensuite \eqref{chap:5:eq.hydro.2.2} dans \eqref{chap:5:eq.hydro.2.1} on obtient finalement
...
\begin{eqnarray}
\lambda(\tau) \simeq \sqrt{\frac{2}{\alpha}}\,\omega_\parallel \, \tau.
\end{eqnarray}
...
Le profil de densité mesuré est représenté en rouge sur la Fig.~\ref{fig:profil}(a), tandis que le profil attendu à l'équilibre dans le piège harmonique est tracé en noir pointillé. Ce dernier est obtenu à partir de l'expression \eqref{chap4:eq.pot.s.1} du potentiel chimique :
...
%			\end{scope}
        % Première figure
        \begin{scope}[shift={(0,0)}]
        	\node (a) at (0,0) {\includegraphics[width=0.45\textwidth]{./figures/05_Disp_Exp/fit_n_vs_tau.pdf}};
        ...
        \begin{scope}[shift={(7,0)}]
        	\node (b) at (0,0) {\includegraphics[width=0.45\textwidth]{./figures/05_Disp_Exp/fit_lambda_vs_tau.pdf}};
    		...
    		\draw[thick, color=colorFour, line width = 0.5ex , <-]
    			(1.9,1) to[out=-90, in=90] (2.2,-0.5)
    			node[below, color = colorFour] { \color{\colorFour} \small TF 1D : $\mu_p \ll \hbar \omega_\perp$};	
        \end{scope}
...
(b) Points bleus : facteurs d'échelle $\lambda(\tau)$ extraits expérimentalement. Courbe orange : résultats numériques obtenus en résolvant les équations hydrodynamiques (eq.~\eqref{chap4:eq.pot.s.1}) avec $\mu_p / \hbar \omega_\perp = 1.4$. Les courbes pointillées montrent les évolutions attendues de $\lambda$ en résolvant l'eq.~\eqref{chap:5:eq.hydro.3.4} pour $\alpha = 1$ (TF 1D) et $\alpha = 1/2$ (TF 3D).
}
...
\subsection{Sonde locale de distribution de rapidités}
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\node[shift={(-3 , 1)}] at (0,0) {\fontsize{12pt}{14pt}\selectfont \bfseries a)};
     	\end{scope}
     	
     	 % Deuxième figure
        \begin{scope}[shift={(7,0)}]
        	\node (b) at (0,0) {\includegraphics[width=0.55\textwidth]{./figures/05_Disp_Exp/profils.pdf}};
        	\node[above=-0.2cm of b , shift={(-2.5 , -1)}] {\fontsize{12pt}{14pt}\selectfont \bfseries b)};
...
%    			node[below, color = colorFour] { \color{\colorFour} \small TF 3D : $\mu_p \gg \hbar \omega_\perp$};	
	...
	\end{tikzpicture}
	\caption{(a) Schéma illustrant la sélection d’une région du nuage atomique, ici représentée par une ellipse bleue illuminée par un faisceau modulé à l’aide du \gls{DMD}.\\
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\begin{scope}[shift={(0,0)}]
			...
			\node (a) at (0,0) {\includegraphics[width=0.3\textwidth]{./figures/05_Disp_Exp/image159.png}};
        ...
        %\draw [color = white] (-3,-1) rectangle +(10,2);
     	...
     	\end{scope}
     	 ...
     	 % Deuxième figure
        \begin{scope}[shift={(7,0)}]
        	\node (b) at (0,0) {\includegraphics[width=0.6\textwidth]{./figures/05_Disp_Exp/image212.png}};
        	\node[above=-0.2cm of b , shift={(-3.2 , -1.3)} , color = white ] {\fontsize{12pt}{14pt}\selectfont \bfseries \color{white} b1)};
        	\node[above=-0.2cm of b , shift={(0.5 , -1.3)} , color = white ] {\fontsize{12pt}{14pt}\selectfont \bfseries \color{white} b2)};
...
\paragraph{Résumé.}  
Nous avons développé une méthode permettant de sonder localement la distribution de rapidités à l’aide du \gls{DMD}. Les atomes extraits par la sélection sont ensuite laissés en expansion le long du guide unidimensionnel. Après un temps d’évolution suffisamment long, la forme du profil de densité obtenu correspond directement à la distribution de rapidités locale. Ce protocole a été testé avec succès sur un gaz initialement préparé à l’équilibre.

\paragraph{Dynamique.}
\subparagraph{Motivation.}
Pour vérifier si le régime asymptotique des expansions longitudinales est atteint, il est nécessaire de comparer les résultats expérimentaux à une théorie dynamique. L’outil adapté est \gls{GHD}, déjà introduite au Chapitre~\ref{chap:GHD}, qui décrit l’évolution spatio-temporelle de la distribution de rapidités.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\begin{scope}[shift={(0,0)}]
			...
			\node (a) at (0,0) {\includegraphics[width=0.5\textwidth]{./figures/05_Disp_Exp/scaling_1.pdf}};
        	\node[above=-0.2cm of a , shift={(-2 , -1)}] {\fontsize{12pt}{14pt}\selectfont \bfseries \color{black} a)};
		...
		\begin{scope}[shift={(8,0)}]
			...
			\node (a) at (0,0) {\includegraphics[width=0.5\textwidth]{./figures/05_Disp_Exp/scaling_2.pdf}};
        	\node[above=-0.2cm of a , shift={(-2 , -1)}] {\fontsize{12pt}{14pt}\selectfont \bfseries \color{black} b)};
...
%     	 % Deuxième figure
%        \begin{scope}[shift={(7,0)}]
%        	\node (b) at (0,0) {\includegraphics[width=0.5\textwidth]{./figures/05_Disp_Exp/image246.png}};
...
\begin{eqnarray}\label{eq.sigma}
	\sigma(\tau) = \sqrt{ \frac{\int \vert \frac{\tau}{\ell} n_{\mathrm{GHD}}( \frac{x-x_0}\tau) - \rho(\frac{x-x_0}\tau) \vert^2 d \tau }{ \int \vert \rho( \frac{x-x_0}\tau) \vert^2 d \tau} }	
\end{eqnarray} avec $n_{\mathrm{GHD}}$ les profils tirés des simulations \gls*{GHD}. Pour $\tau=50$\,ms, l’écart-type reste d’environ $12\%$, et une convergence complète nécessiterait des temps irréalistes ($\sim 500$\,ms). L’extraction de la distribution de rapidité à partir des profils reste donc approximative.

\smallskip

Dans la Fig ~\ref{fig:GHD1} a), on compare les profils expérimentaux obtenus après une expansion de différentes durées à ceux issus des simulations \gls{GHD} pour une distribution thermique initiale. L’écart-type $\sigma(\tau)$ (eq.~\eqref{eq.sigma}) est tracé en fonction du temps d’expansion dans la Fig ~\ref{fig:GHD1} b). On constate que le régime asymptotique n’est pas encore atteint pour le temps d’expansion utilisé expérimentalement ($\tau = 40$\,ms). Et il faudrait des temps d’expansion bien plus longs pour que l’écart-type diminue significativement.
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\begin{scope}[shift={(0,0)}]
			...
			\node (a) at (0,0) {\includegraphics[width=0.7\textwidth]{./figures/05_Disp_Exp/image255.png}};
        	\node[above=-0.2cm of a , shift={(-3.2 , -1.8)}] {\fontsize{12pt}{14pt}\selectfont \bfseries \color{black} a)};
        ...
        \begin{scope}[shift={(4,2)}]
        	\node (b) at (0,0) {\includegraphics[width=0.3\textwidth]{./figures/05_Disp_Exp/image265.png}};
        	\node[above=-0.2cm of b , shift={(1.2 , -0.7)} ] {\fontsize{12pt}{14pt}\selectfont \bfseries \color{colorOne} b)};
	...
	\end{tikzpicture}
	\caption{(a) La tranche , centrée en $x_0 = 0\,\mu\mathrm{m}$, subit une expansion le long de l'axe longitudinal et d'une largeur $\ell = 37\,\mu\mathrm{m}$. Le profil initial du gaz est montré en gris. Les variations de densité pour différents temps d'expansion sont représentées par des courbes colorées. Les lignes pointillées noires indiquent les résultats des simulations \gls{GHD} pour une distribution de rapidités initialement thermique, paramétrée par la température $T$ et le potentiel chimique $\mu(x_0)  = \mu$, tels qu'ils sont extraits du profil de densité in situ.(b) L'écart-type $\sigma(\tau)$ (eq.~\eqref{eq.sigma})  est calculé entre la distribution de rapidités et les résultats des simulations \gls{GHD} en fonction du temps d'expansion $\tau$. La ligne verticale noire correspond au temps d'expansion utilisé dans l'expérience.
}
...
Le faisceau pousseur (\gls{DMD}) permet non seulement de sonder la distribution de rapidités locale, mais aussi de créer des situations hors équilibre. Un protocole illustratif consiste à retirer les atomes d’une région centrale de taille $\ell_0 = 70\,\mu$m, puis à laisser le gaz évoluer librement longitudinalement (1D). Après un temps $\tau = 15$\,ms, la distribution de rapidités développe une structure doublement piquée, très différente d’une distribution thermique. Une seconde sélection locale ($\ell=35\,\mu$m) suivie d’une expansion révèle expérimentalement ce profil, en excellent accord avec les simulations \gls{GHD} (Fig ~\ref{fig:GHD2}).
...
\begin{figure}[H]
	...
	\begin{tikzpicture}
		...
		\begin{scope}[shift={(0,0)}]
			...
			\node (a) at (0,0) {\includegraphics[width=0.6\textwidth]{./figures/05_Disp_Exp/image295.png}};
        	
        	\node[above=-0.2cm of a , shift={(-3.7 , -0.7)}] {\fontsize{6pt}{14pt}\selectfont \bfseries \color{colorOne} a1)};
        	\node[above=-0.2cm of a , shift={(-0.9 , -0.7)}] {\fontsize{6pt}{14pt}\selectfont \bfseries \color{colorOne} a2)};
        	\node[above=-0.2cm of a , shift={(1.8 , -0.7)}] {\fontsize{6pt}{14pt}\selectfont \bfseries \color{colorOne} a3)};
        	
        	\node[above=-0.2cm of a , shift={(-3.7 , -1.7)}] {\fontsize{6pt}{14pt}\selectfont \bfseries \color{colorTwo} a4)};
        	\node[above=-0.2cm of a , shift={(-0.9 , -1.7)}] {\fontsize{6pt}{14pt}\selectfont \bfseries \color{colorTwo} a5)};
        	\node[above=-0.2cm of a , shift={(1.8 , -1.7)}] {\fontsize{6pt}{14pt}\selectfont \bfseries \color{colorTwo} a6)};
        ...
        %\draw [color = white] (-3,-1) rectangle +(10,2);
     	...
     	\end{scope}
     	 ...
     	 % Deuxième figure
        \begin{scope}[shift={(7,0)}]
        	\node (b) at (0,0) {\includegraphics[width=0.4\textwidth]{./figures/05_Disp_Exp/image290.png}};
        	\node[above=-0.2cm of b , shift={(-2.2 , -0.8)} ] {\fontsize{12pt}{14pt}\selectfont \bfseries \color{black} b)};
...
Cette approche démontre l’importance d’une mesure locale : alors que la distribution globale $\int dx\,\rho(x,\theta)$ est conservée, la distribution locale $\rho(x,\theta)$ reflète directement la dynamique hors équilibre. Le dispositif basé sur le \gls{DMD} offre ainsi un outil puissant pour générer et caractériser divers protocoles hors équilibre (cisaillement, jonctions de densité, états stationnaires non thermiques, etc.), ouvrant de nouvelles perspectives pour l’étude des gaz de Bose 1D. Pour une présentation plus détaillée de ces résultats, le lecteur pourra se référer à notre article~\cite{PhysRevLett.133.113402}.


\section*{Résumé du chapitre}

Dans ce chapitre, nous avons décrit les principaux dispositifs expérimentaux utilisés dans notre étude. Le système atomique est préparé dans une géométrie unidimensionnelle, avec un confinement longitudinal réalisé par un potentiel de la forme
\(
V_{\parallel}(x) = a_0 + a_1 x + a_2 x^2 + a_3 x^3 + a_4 x^4 ,
\)
qui permet d’ajuster finement la forme du piège.  
Par ailleurs, l’utilisation du \gls{DMD} joue un rôle essentiel : cet outil permet non seulement de sélectionner 
localement une portion du nuage atomique afin de sonder la distribution des rapidités, mais également de générer 
des dynamiques hors équilibre. Ces protocoles incluent, par exemple, l’analogue du \emph{pendule de Newton}, 
ainsi que la situation présentée dans la dernière section. 
