...
Pour un systemps dans les regime \gls{qBEC} ( pour une paramètre de Lieb definie en \ref{chap2:eq:gamma.1} , $\gamma  \ll 1$ et $T \sim 0$), la dynamique du système étudié est décrite par l’équation de \gls{GP} \eqref{chap.1:eq.GP.2} : 
\begin{eqnarray*}
...
\)
En négligeant le terme de pression quantique $Q(n)$, ces équations sont équivalentes aux deux premières de \eqref{chap:3:eq:hydro.1}, en tenant compte de la relation thermodynamique $dP = n \, d\mu$:
...
En utilisant q'à l'équilibre \(\mu\big(n_0(y)\big)=\mu_0-\tfrac12 \omega_\parallel^2 y^2\), soit \(\mu'(n_0(y))\,n_0'(y)=-\omega_\parallel^2 y\) soit 
...
\begin{enumerate}
    \item Une zone du nuage atomique de taille \(\ell\) centrée en \(x_0\) est sélectionnée à l’aide du \gls{DMD}. La pression de radiation supprime instantanément les atomes en dehors de la zone, laissant uniquement ceux de la cellule.
