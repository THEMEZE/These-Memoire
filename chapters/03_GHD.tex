...
\begin{equation}\label{chap:GHD:eq.chochet.bonnemain.1}
	...
	\right],
\end{equation}
où $\nu$ désigne la fonction d’occupation.  
...
\end{equation}
où $\rho(x,\theta)$ est la densité de rapidité.  
...
La valeur moyenne $\langle \mathcal{Q}[f] \rangle_{\varrho[w]}$ a été définie en~\eqref{chap.TBA.moy.dens}.  
La matrice densité locale $\varrho^{(\mathcal{S})}[w]$ a été introduite en~\eqref{chap.2.densite.1}.  
De manière analogue, la \emph{matrice densité globale} $\varrho[w]$ s’écrit
\begin{equation}\label{chap:GHD:eq.charge.global.2}
	\varrho[w] 
	= \frac{1}{Z[w]}\, e^{-\mathcal{Q}[w]}, 
	\qquad  
	Z[w] = \mathrm{Tr} \left[ e^{-\mathcal{Q}[w]} \right],
\end{equation}
où la charge globale $\mathcal{Q}[w]$ est définie par~\eqref{chap:GHD:eq.charge.global.1}, et $w$ désigne le poids spectral.  

Cette formulation met en évidence le lien entre la description statistique du système et la conservation des charges globales, en généralisant le principe de Gibbs aux systèmes intégrables par l’introduction de l’ensemble d’observables $\mathcal{Q}[f]$ sur l’espace spectral.
...
\[
\underset{\mathrm{therm}}{\lim} \, \langle \mathcal{O} \rangle_{\varrho[w]} \; \equiv \; \mathcal{O},
\]
...
\end{equation}
toutes trois étant des fonctions de $\rho(x,\theta,t)$.
...
Dans ce modèle, la vitesse effective et l’accélération effective se réécrivent :
\begin{equation}
...
\begin{equation*}
	\{F,G\} = \iint dx\,d\theta\;\frac{\nu}{2\pi}\,\left[\partial_x \!\left( \frac{\delta F}{\delta \rho} \right)\, \partial_\theta \!\left( \frac{\delta G}{\delta \rho} \right) - \partial_x \!\left( \frac{\delta G}{\delta \rho} \right) \, \partial_\theta \!\left( \frac{\delta F}{\delta \rho} \right) \right].
\end{equation*}

Les flux et termes de force \eqref{chap:GHD:eq.conserv.2.1} s’expriment alors en remplaçant la vitesse effective $v^{\mathrm{eff}}$ et l’accélération effective $a^{\mathrm{eff}}$ par leurs expressions issues de la dynamique hamiltonienne libre :
\[
	v^{\mathrm{eff}} \to \dot{x} = \partial_\theta h, 
	\quad a^{\mathrm{eff}} \to \dot{\theta} = -\partial_x h.
\]
