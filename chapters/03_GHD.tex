...
\end{equation}
toutes trois étant des fonctions de $\rho(\cdot,\cdot,t)$ et l’opération d’\emph{habillage} (notée $^{\mathrm{dr}}$) a été définie au chapitre~\ref{chap:LL-BA}, Eq.~\eqref{eq:dressing}.

\medskip

Les équations~\eqref{chap:GHD:eq.Liouv.1}, \eqref{chap:GHD:eq.conserv.2} et \eqref{chap:GHD:eq.conserv.3} décrivent la dynamique au régime d’Euler. En dehors de cette approximation, il est nécessaire de prendre en compte des contributions supplémentaires associées aux effets diffusifs~\cite{DeNardis2018}.
...
\end{eqnarray}
où $\Delta$ désigne le décalage en diffusion défini dans le modèle \gls{LL} en Eq.\eqref{eq:I-1-16}
...
En partant de la l'écriture de la fonction d'ocumation en \eqref{chap:GHD:eq.nu.v.a.1} et en utilisant la définition de l'appliocation \emph{dressing} \eqref{eq:dressing}, il vient que 
...
En dérivant la définition de l'opérateur \emph{habillé} \eqref{eq:dressing}, on obtient la relation suivante :
...
{\color{blue}
Dans  \cite{bonnemain2024hamiltonian}, il est montré que les équation GHD peuvent
...
et je présente ici ce résultat. En aucun cas les calculs ci-dessous ne sont une dérivation de la GHD.}
...
\begin{equation}\label{chap:GHD:eq.chochet.bonnemain.1}
	...
	\right],
\end{equation}
où $\nu$ désigne la fonction d’occupation. Dans ce crochet l'application de l’opérateur \emph{habillé} $(\cdot)^{\mathrm{dr}}_{[\nu]}$ (introduit dans \eqref{eq:dressing})  traduit les interactions entre particules.
...
La vitesse effective et l'accélération effective \eqref{chap:GHD:eq.nu.v.a.1} interviennent dans les équations de mouvement
...
En utilisant la définition de $veff$ et $aeff$ données eq. xx, on trouve que le crochet \eqref{chap:GHD:eq.chochet.bonnemain.3} appliqué à $(f,h)$ devient 
...
{\color{blue}
  Non, on ne retrouve pas du tout les équations d'Euler classiques !! ON n'a pas d''équation fermée pour $q_{[1]}$ et $q_{[Id]}$ !! }
\begin{eqnarray}\label{chap:3:eq:hydro.1}
	...
	\right .  
\end{eqnarray}
avec la densité de particule $n(x, t) = q_{[1]}$, la vitesse moyenne du fluide $u = \frac{q_{[\theta]}}{n}$ , la pression cinétique du fluide $\mathcal{P}(x, t) = \left( q_{[\theta^2]} - \frac{q_{[\theta]}^2}{n} \right)$ , l'énergie totale $E = nu^2/2 + nV + ne$ où $e(x,t)$ est l'énergie interne d'une particule {\color{blue} ?????????????????}.
...
{\color{blue} ??? Cette structure est l’analogue classique de la description en termes de {\bf distribution de rapidité} dans le cadre intégrable.} Elle constitue le point de départ naturel pour développer une description hydrodynamique généralisée (GHD) dans le cas intégrable.
...
\section*{Interprétation de l'opérateur de \textit{dressing} en GHD}
