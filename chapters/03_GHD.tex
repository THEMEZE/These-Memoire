...
\begin{eqnarray}\label{chap:3:eq:hydro.1}
	...
	\right .  
\end{eqnarray}
avec la densité de particule $n = q_{[1]}$, la vitesse moyenne du fluide $u = {q_{[\theta]}}/{q_{[1]}}$ , la pression cinétique du fluide $\mathcal{P} =  q_{[\theta^2]} - {q_{[\theta]}^2}/{q_{[1]}} $ , l'énergie totale $E = nu^2/2 + nV + ne_{\rm int}$ où $e_{\rm int} = q_{[\theta^2/2]}$ est l'énergie interne d'une particule {\color{blue} ?????????????????}.
...
{\color{blue} ??? Cette structure est l’analogue classique de la description en termes de {\bf distribution de rapidité} dans le cadre intégrable.} Elle constitue le point de départ naturel pour développer une description \gls{GHD} dans le cas intégrable.
