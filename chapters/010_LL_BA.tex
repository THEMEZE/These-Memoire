...
Le cadre est ensuite étendu au cas général de \(N\) particules, permettant ainsi de dériver les équations de Bethe pour un système interactif. Ces équations encapsulent toute l’information sur les états propres du système.
...
Pour chaque sous-espace propre on impose que les états propres forment une base orthonormale \ie en utilisant \eqref{chap.1.rapel.libre.5}, les fonctions propres $f_{k}$ écrites sous la forme \eqref{chap.1.rapel.libre.4}, sont orthogonales avec $\overline{f}_{k} \colon x \mapsto \pm ( b^\ast e^{-ikx} - a^\ast e^{ikx} )$  soit $(\overline{f}_{k} , \varphi_{k} ) = 0$, et on impose que  $ \vert a \vert^2 + \vert b \vert^2 = L^{-1}$ pour assurer la normalité  de $\varphi_{k}$  et de $\varphi_{\overline{k}}$ soit  $( f_{k} , f_{k} ) = (\overline{f}_{k} , \overline{f}_{k})   = 1$. 
...
\begin{eqnarray}\label{eq.chap.1.Q.P.k.2}
	\operator{Q} \ket{\{k_a\}} = \left( \sum_{i = 1}^N 1 \right) \ket{\{k_a\}},\quad \operator{P} \ket{\{k_a\}} = \hbar \left( \sum_{i = 1}^N k \right) \ket{\{k_a\}}.
\end{eqnarray}
...
\)
jouant ici un rôle analogue à celui d’un pseudo-moment associé dans le référentiel de laboratoire.
...
\(
	...
	e^{i\theta_{1}L}\;b = a,
\)
c’est‑à‑dire en utilisant \eqref{eq:I-1-10-3}, les \textbf{équations de Bethe à deux particules} :
...
En comparaison avec le cas d’une seule particule (où la condition aux bords périodiques impose simplement $\theta L = 2\pi n$ ~\eqref{chap.1.rapel.libre.5}), on voit que les équations de Bethe à deux particules \eqref{eq:PC2}--\eqref{eq:Bethe2} incorporent la fonction $\Phi$, qui encode explicitement l’\textbf{interaction entre particules}.
...
Tous les nombres $I_a$ sont distincts. Cette propriété établit un lien direct avec un gaz de \textbf{fermions libres} en une dimension : bien que les particules du modèle de Lieb–Liniger soient bosoniques, la structure des équations de Bethe impose une exclusion effective au niveau des nombres quantiques. Pour cette raison, on parle de \textbf{nombres quantiques fermioniques}.  
L’équation de Bethe \eqref{chap:1:eq:EBA} définit ainsi une bijection entre une configuration de rapidité $\{ \theta_a \}$ (\gls{BA}) et un ensemble de nombres quantiques $\{ I_a \}$ (\gls{FBA}) \cite{Girardeau_1960}.
...
Ce système d'équations détermine les états propres du système de \gls{LL}  en volume fini, et joue un rôle fondamental dans la description exacte de ses propriétés thermodynamiques et dynamiques.
...
Dans la limite thermodynamique $(\underset{\mathrm{\tiny therm}}{\lim} \equiv \underset{\underset{ N/L = \mathrm{cnst}}{L,N \to \infty,\infty}}{\lim} )$ , les valeurs de \( \theta_a \) deviennent quasi-continues, avec un espacement \( \theta_{a+1} - \theta_a = \mathcal{O}(1/L) \), et se condensent dans un intervalle symétrique autour de zéro $[-\theta_{\mbox{\tiny max}} , \theta_{\mbox{\tiny max}} ]$ . En supposant l'ordre \( I_a \geq I_b  \) implique \(\theta_a \geq \theta_b \), cet intervalle constitue ce qu'on appelle la {\em mer de Fermi} (ou sphère de Fermi en dimension un).

\medskip

\paragraph{Distribution de rapidité $\rho(\theta)$.}

Nous introduisons la {\bf distribution de rapidité macroscopique} par unité de longueur, notée \( \rho(\theta) \), et définie par
\begin{eqnarray}
...
\begin{figure}[h]
    ...
    \caption{
	Illustration de la relation entre la densité d’états $\rho_s(\theta)$ et les nombres quantiques fermioniques $I_a$.
    Rapiditées dans l’intervalle $[\theta, \theta + \delta\theta]$ s’associent à un ensemble de nombres quantiques fermioniques situés dans $[I, I + \delta I]$. La densité d’états $\rho_s$ se déduit alors de la relation $\delta I \simeq L \rho_s(\theta) \delta \theta $
    }
    \label{fig:rho_s.I}
\end{figure}
...
\)
donne le nombre total de particules dans le système. Le passage de la somme discrète à l'intégrale dans le second membre de l'équation de Bethe s'effectue en utilisant la correspondance:
\begin{eqnarray}
...
\end{eqnarray}
et, comme \( \rho = \rho_s \), à l'état fondamental, on obtient l’équation linéaire intégrale satisfaite par la distribution de rapidités :
...
Le chapitre suivant sera consacré à l’introduction du \gls{GGE} et à l’écriture explicite du formalisme de Bethe Ansatz thermodynamique  (\acrshort{TBA}), qui fournit un cadre puissant pour décrire les propriétés thermodynamiques et hors équilibre des systèmes intégrables.
