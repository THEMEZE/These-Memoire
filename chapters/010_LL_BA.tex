...
Dans ce chapitre, nous avons introduit le modèle de \glsreset{LL}\gls{LL} , qui décrit un gaz de bosons unidimensionnel soumis à une interaction de contact. Dans ce cadre, les grandeurs conservées naturelles ne sont pas seulement le nombre total de particules $\operator{Q}$ et la quantité de mouvement totale $\operator{P}$, mais également l’énergie associée à l’Hamiltonien de \gls{LL}, $\operator{H}$, (plutôt qu’à la simple énergie cinétique $\operator{K}$). Ces charges conservées correspondent respectivement aux moments d’ordre $0$, $1$ et $2$ en rapidité. Comme nous le verrons au chapitre suivant, des charges d’ordre supérieur interviennent dans la description des états en équilibre, notamment via l’Ensemble de Gibbs Généralisé (\gls{GGE}).

\medskip

L’imposition de conditions aux bords périodiques conduit naturellement aux équations de \glsreset{BA}\gls{BA} \eqref{chap:1:eq:EBA}. Dans la limite thermodynamique, celles-ci se condensent en une équation intégrale reliant la densité d’états $\rho_s(\theta)$, la densité de particules occupées $\rho(\theta)$, la densité de trous $\rho_h(\theta)$, ainsi que la fonction d’occupation $\nu(\theta)$. Le \glsentrylong{BA} établit ainsi le lien fondamental entre ces différentes grandeurs et constitue la base de la description thermodynamique du modèle \eqref{eq:TBA-rhos-2}.

\medskip

Le chapitre suivant sera consacré à l’introduction du \gls{GGE} et à l’écriture explicite de le \gls{TBA}, qui fournit un cadre puissant pour décrire les propriétés thermodynamiques et hors équilibre des systèmes intégrables.
