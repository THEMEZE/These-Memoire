...
%Nous passons ensuite à la formulation du problème en champ quantique, en exprimant l'Hamiltonien en termes d’opérateurs de création et d’annihilation dans la représentation positionnelle : il s’agit du passage à la seconde quantification. Cette étape permet de formaliser clairement les termes à un corps et à deux corps dans l’Hamiltonien, et d’établir les notations qui seront utilisées tout au long du chapitre.

\medskip
Une fois ce cadre posé, nous généralisons le raisonnement au cas de \(N\) particules pour introduire le modèle complet de \acrlong{LL}. Nous présentons alors l'Ansatz de Bethe (\gls{BA}) dans sa forme générale, qui fournit les états propres de l’Hamiltonien. Ce formalisme permet d’accéder explicitement au spectre du système, ainsi qu’à diverses quantités physiques telles que l’impulsion totale et le nombre de particules.
...
Le cadre est ensuite étendu au cas général de \(N\) particules, permettant ainsi de dériver les équations de \gls{BA} pour un système interactif. Ces équations encapsulent toute l’information sur les états propres du système.
...
La mécanique quantique se développe historiquement en deux grandes étapes : la \emph{première quantification}, aussi appelée quantification canonique et la \emph{seconde quantification}. Comprendre ces deux cadres est essentiel pour aborder les systèmes quantiques complexes, en particulier ceux où le nombre de particules peut varier (cf annex \ref{annex:Fourier}).
...
% 	{~}\\
\Example{
        particule libre en une boite à une dimension.
		%\subparagraph{Exemple : particule libre en une boite à une dimension.} 
    }{   
...
Les fonctions propres spatiales $f_k(x)$ de l’hamiltonien libre s’écrivent comme des combinaisons linéaires d’ondes planes  
	\begin{eqnarray}
		f_k(x) = a e^{-i k x} + b e^{i k x},~~ \mbox{avec}\quad  (a,b) \in \mathbb{C}^2\label{chap.1.rapel.libre.4}.
	\end{eqnarray}
\subparagraph{Périodicité.}
	Si la particule est confinée dans une boîte de longueur $L$ avec des conditions aux limites périodiques (\ie $f_k(x+L) = f_k(x)$), alors le spectre de $k$ est quantifié : 
	\begin{eqnarray}
		e^{ikL}= 1 \quad\mbox{ou encore} kL \in 2\pi\mathbb{Z}\label{chap.1.rapel.libre.5}.
	\end{eqnarray}
	Le problème est équivalent à celui d’une particule libre sur un cercle de périmètre $L$.\\
	
	\smallskip
	
	La particule est délocalisée sur tout l’espace (le cercle), sans structure particulière \ie les solutions \eqref{chap.1.rapel.libre.4} correspondent à des {\bf états non liés} (ou états de diffusion).

	Les solutions \eqref{chap.1.rapel.libre.4} avec la condition de périodicité \eqref{chap.1.rapel.libre.5} décrivent une particule libre se propageant sur un cercle, sans potentiel externe.
...
Pour $k \neq 0 $ (respectivement pour $k = 0$), la fonction propre $f_k(x)$ de l’équation \eqref{chap.1.rapel.libre.4} appartient à un sous-espace propre associé à $k$ de dimension 2 (respectivement de dimension 1) engendré par $x \mapsto e^{-ikx}$ et $x \mapsto e^{ikx}$ (respectivement par $x \mapsto 1$).
...
\begin{eqnarray}
	( f_{k'} , f_{k} ) = \int_0^L f_{k'}^\ast(x) f_{k}(x) \, dx .\label{chap.1.rapel.libre.6}
\end{eqnarray}
Les sous-espaces propres sont orthogonaux entre eux \ie en utilisant les conséquences de la condition de périodicité \eqref{chap.1.rapel.libre.5}, $( f_{k'} , f_{k} ) = 0$ pour $\vert k' \vert \neq \vert k \vert $. 
Pour chaque sous-espace propre, on impose que les états propres forment une base orthonormale \ie en utilisant \eqref{chap.1.rapel.libre.5}, les fonctions propres $f_{k}$ écrites sous la forme \eqref{chap.1.rapel.libre.4}, sont orthogonales avec $\overline{f}_{k} \colon x \mapsto \pm ( b^\ast e^{-ikx} - a^\ast e^{ikx} )$  soit $(\overline{f}_{k} , f_{k} ) = 0$, et on impose que  $ \vert a \vert^2 + \vert b \vert^2 = L^{-1}$ pour assurer la normalité  de $f_{k}$  et de $f_{\overline{k}}$ soit  $( f_{k} , f_{k} ) = (\overline{f}_{k} , \overline{f}_{k})   = 1$. 

\medskip

Les solutions générales de l’équation de Schrödinger s’écrivent alors comme une superposition d’états propres  $\psi = c_0 \psi_0 +  \sum_{\vert k \vert > 0 } (c_k \psi_k  + \overline{c}_{k} \overline{\psi}_{k}) $. 
...
%	\end{eqnarray}
	\begin{eqnarray}\label{chap.1.rapel.libre.7}
		\varphi_{\pm k}   & = & \displaystyle \frac{1}{\sqrt{L}} e^{\pm ikx} 
	\end{eqnarray}
	Ces derniers en plus d'être états propres de l’opérateur énergie $\operator{\mathcal{H}}$, sont des états propres de l’opérateur impulsion $\operator{p}$, avec valeurs propres opposées $\pm \hbar k$.
	\item {\bf Base symétrique / antisymétrique :} En appliquant la matrice de passage unitaire  $\frac{1}{\sqrt{2}}\left (\begin{matrix}1 & 1 \\ -i & + i\end{matrix}\right)$ à la base $\{ \varphi_{+k} , \varphi_{-k} \}$, on passe dans la base    
%	\begin{eqnarray}
...
Cette condition d’orthonormalitée est imposée afin de garantir l’indépendance linéaire des états quantiques, et d'assurer que toute fonction d’onde de l’espace de Hilbert puisse être développée de manière unique sur cette base. 
...
\begin{eqnarray}\label{chap.1:eq.rapel.etat.prim.1}
	...
	\ket{x} = \sum_{L k \in 2 \pi \mathbb{Z}}  \varphi_k^\ast(x) \ket{k},
\end{eqnarray}
...
\) 
avec $\operator{S}_N $  le groupe symétrique d'ordre $N$, ou encore, de manière équivalente :
...
\subparagraph{Dans la base discrète des modes \( \{ \ket{k} \} \).}

Soit \( \operator{f} \) un opérateur à une particule, dont les éléments de matrice dans une base orthonormée \( \{ \ket{k} \} \) sont donnés par \( f_{\lambda\nu} = \langle \lambda \vert \operator{f} \vert \nu \rangle \). Un opérateur symétrique à \( N \) particules correspondant à la somme des actions de \( \operator{f} \) sur chacune des particules s’écrit :
\(
...
L'opérateur $\operator{a}^\dagger_\lambda \operator{a}_\nu^{}$ fait la transition d'une particule de l'état $\nu$ à l'état $\lambda$. Si $\lambda = \nu$ cet opérateur est l'opérateur nombre de particules dans le mode $\lambda$.
...
% 	]
\Example{
	Énergie cinétique totale des particules libres.
}
{
...
En injectant 
\(
	\bra{x} \operator{f} \ket{y} = - \frac{\hbar^2}{2m} \partial_{y}^2 \delta ( y - x )
\) 
...
Lorsque cet Hamiltonien agit sur l’état de Fock à $N$ particules libres $\ket{\{k_a\}}$, les règles de commutation (\ref{chap:1:com.1}) ainsi que la définition des états de Fock (\ref{chap:eq.vide.fock}) impliquent (cf. Annexe \ref{annex:PH}) :
\begin{eqnarray}\label{eq.chap.1.cinietique.4}
\operator{K}\ket{ \{k_a\}} =  \int_0^L d^N z \, \operator{\mathcal{K}}_N \, \varphi_{\{k_a\}}(\{z_a\} ) \operator{\Psi}^\dag(z_1) \cdots \operator{\Psi}^\dag(z_N) \ket{\emptyset}\, , \quad \mbox{avec} \quad \operator{\mathcal{K}}_N = -\sum_{i=1}^N \frac{\hbar^2\operator{\partial}_{z_i}^2}{2m}, 
\end{eqnarray}
...
\begin{equation}
	\operator{Q} = \sum_{k}  \operator{b}^\dagger_k \operator{b}_k^{} , \quad  \operator{P} = \hbar\sum_{k} k \, \operator{b}^\dagger_k \operator{b}_k^{},\label{eq.chap.1.Q.P.k.1}
\end{equation}
Lorsqu’on les applique à un état de Fock à $N$ particules \(\ket{\{k_a\}}\), on obtient :
\begin{eqnarray}\label{eq.chap.1.Q.P.k.2}
	\operator{Q} \ket{\{k_a\}} = \left( \sum_{i = 1}^N 1 \right) \ket{\{k_a\}},\quad \operator{P} \ket{\{k_a\}} = \hbar \left( \sum_{i = 1}^N k_i \right) \ket{\{k_a\}}.
\end{eqnarray}
...
\begin{eqnarray}
	\operator{Q}\ket{ \{k_a\}} =  \int_0^L d^N z \, \operator{\mathcal{N}}_N \, \varphi_{\{k_a\}}(\{z_a\} ) \operator{\Psi}^\dag(z_1) \cdots \operator{\Psi}^\dag(z_N) \ket{\emptyset}\, , \quad &\mbox{avec}& \quad \operator{\mathcal{N}}_N = \sum_{i=1}^N 1,\label{eq.chap.1.Q.1}\\
	\operator{P}\ket{ \{k_a\}} =  \int_0^L d^N z \, \operator{\mathcal{P}}_N \, \varphi_{\{k_a\}}(\{z_a\} ) \operator{\Psi}^\dag(z_1) \cdots \operator{\Psi}^\dag(z_N) \ket{\emptyset}\, , \quad &\mbox{avec}& \quad \operator{\mathcal{P}}_N = -i\hbar \sum_{i=1}^N \operator{\partial}_{z_i}\label{eq.chap.1.P.1}.
\end{eqnarray}
...
% 	]
\Information{
\subparagraph{On s'avance sur le chapitre (\ref{chap:relaxation}).} , en voulant généraliser avec $\operator{f} = \operator{p}^q$ où $q$ est un entier. Soit dans la base \(\{\ket{k_a}\}\) : \(\operator{F} = \hbar^q \sum_k k^q \operator{b}_k^\dagger \operator{b}_k^{} \) et en l'appliquant à \(\ket{\{k_a\}}\) :
...
\(
	\operator{F}\ket{ \{k_a\}} =  \int_0^L d^N z \, \operator{\mathcal{F}}_N \, \varphi_{\{k_a\}}(\{z_a\} ) \operator{\Psi}^\dag(z_1) \cdots \operator{\Psi}^\dag(z_N) \ket{\emptyset}\, 
\) \mbox{avec} 
\(\operator{\mathcal{F}}_N = (-1)^q\hbar^q \sum_{i=1}^N (\operator{\partial}_{z_i})^q.
\)
...
%On remarque que l’opérateur $\operator{K}$ (énergie cinétique) ne coïncide avec l’énergie totale que pour un gaz de particules libres, sans interaction ni potentiel externe. Dans ce cas, les états de Fock $\ket{{k_a}}$ sont propres de $\operator{K}$, ainsi que de tous les opérateurs $\operator{F}$ définis pour $q \geq 0$.
%
%\medskip
%
%Lorsque des interactions entre particules sont introduites, les états libres $\ket{{k_a}}$ ne sont plus propres de l’Hamiltonien complet (cinétique + interaction), ni des opérateurs $\operator{F}$. On introduit alors les états de Bethe $\ket{{\theta_a}}$, qui sont simultanément propres de l’Hamiltonien complet, des opérateurs $\operator{Q}$ et $\operator{P}$, ainsi que de l’ensemble des charges conservées d’ordre supérieur.
%
%\medskip
%
%Pour rendre compte des interactions, il est nécessaire d’enrichir la construction de $\operator{F}$ en y incluant des contributions à deux corps (et plus généralement à plusieurs corps), ce qui conduit précisément aux charges locales d’intégrabilité.

L’opérateur $\operator{K}$ (énergie cinétique) ne représente l’énergie totale que dans le cas d’un gaz libre, sans interaction ni potentiel externe. Dans ce régime, les états de Fock $\ket{{k_a}}$ sont propres de $\operator{K}$ ainsi que de tous les opérateurs $\operator{F}$ construits pour $q \geq 0$.

\medskip

Dès que des interactions entre particules sont introduites, ces états libres ne sont plus propres de l’Hamiltonien complet (cinétique + interaction), ni des opérateurs $\operator{F}$. Il faut alors considérer les états de Bethe $\ket{{\theta_a}}$, qui diagonalisent l’Hamiltonien complet, mais aussi les charges conservées fondamentales ($\operator{Q}$ et $\operator{P}$) et l’ensemble infini de charges d’ordre supérieur.

\medskip

Dans ce cadre, les opérateurs $\operator{F}$ ne suffisent plus : il est nécessaire d’introduire des contributions à plusieurs corps pour tenir compte des interactions, ce qui conduit naturellement à la hiérarchie des charges locales d’intégrabilité.
		}
% \end{mdframed}
...
\(
    ...
    \label{eq:V2corps}.
\)
où $\operator{v}^{(i, j)}$ désigne l’interaction à deux corps entre les $i^\text{e}$ et $j^\text{e}$ particules , exprimés dans la base à deux états :
\(
...
En injectant les relations des changements de base d'état
...
\end{eqnarray}
et quand on l'applique à l'état $\ket{\{k_a\}}$, les règles de commutations (\ref{chap:1:com.1}) et la définition d'état de Fock (\ref{chap:eq.vide.fock}) impliquent que (cf Annexe \ref{annex:PH})
\begin{eqnarray}\label{chap.1:eq.rapel.opp.2.second.ponctuel.2}
\operator{V}\ket{\{k_a \}} =  \int d^Nz \, \operator{\mathcal{V}}_N \varphi_{\{k_a\}}( \{z_a\} )\operator{\Psi}^\dag(z_1)\cdots \operator{\Psi}^\dag(z_N) \ket{\emptyset} \, \quad \mbox{avec} \quad  \operator{\mathcal{V}}_N 	= g\sum_{1\leq i < j \leq N } \operator{\delta}(z_i - z_j)	
\end{eqnarray}
%avec 
%\(
...
\subsubsection{Expression de l’Hamiltonien de Lieb-Liniger. }\label{chap1:sssec:LL}
...
\begin{eqnarray}\label{chap.1:eq.LL.champ.2}
	\operator{H}\ket{\{\theta_a\}} =  \int d^Nz \, \operator{\mathcal{H}}_N \varphi_{\{\theta_a\}}(\{z_a\})\operator{\Psi}^\dag(z_1)\cdots \operator{\Psi}^\dag(z_N) \ket{\emptyset}, 
\end{eqnarray}
avec $\{\theta_a\} \equiv \{ \theta_1 , \cdots , \theta_N \} $ et
\begin{eqnarray}\label{chap.1:eq.LL.eta.1}
\operator{\mathcal{H}}_N  =  \operator{\mathcal{K}}_N  +  \operator{\mathcal{V}}_N \, \quad \mbox{où on rappelle} \quad \operator{\mathcal{K}}_N = -\frac{1}2\sum_{i=1}^N \partial_{z_i}^2 , \,  \mbox{et} \, 	\operator{\mathcal{V}}_N = g\sum_{1\leq i < j \leq N } \operator{\delta}(z_i - z_j)	. 
\end{eqnarray}
...
\begin{eqnarray}\label{chap.1:eq.GP.2}
...
\begin{itemize}[label = $\bullet$]
    \item Une interaction \textbf{locale et à deux corps} de type $g \, \operator{\Psi}^\dagger \operator{\Psi}^\dagger \operator{\Psi} \operator{\Psi}$ (cas du modèle de \gls{LL}) est invariante par $U(1)$ et par translation. Ainsi, $\operator{Q}$ et $\operator{P}$ sont conservés.  
...
Dans le cas particulier du modèle de \gls{LL}, les opérateurs nombre total de particules $\operator{Q}$ et quantité totale de mouvement $\operator{P}$ commutent avec l’Hamiltonien $\operator{H}$ :  
\begin{eqnarray}
...
%     rightmargin=2pt
% 	]	
	%Nous verrons au chapitre~\ref{chap:relaxation} que, dans les systèmes intégrables comme le modèle de \gls{LL}, cette situation s’étend à une {\bf \em infinité d’intégrales du mouvement}. 
	%Cette richesse supplémentaire de conservation a des conséquences profondes sur la dynamique hors d’équilibre, en particulier sur la relaxation vers des états stationnaires.
...
\Information{
	Nous verrons au chapitre~\ref{chap:relaxation} que, dans les systèmes intégrables tels que le modèle de \gls{LL}, cette situation se généralise à l’existence d’une {\bf\em infinité d’intégrales du mouvement}.
	Cette richesse supplémentaire de quantités conservées a des conséquences profondes sur la dynamique hors d’équilibre, notamment sur les mécanismes de relaxation vers des états stationnaires.
	Elle mène naturellement à l’introduction de l’ensemble de Gibbs généralisé, ou \acrshort{GGE} (\acrlong{GGE}).
}
% \end{mdframed}


\paragraph{États propres et valeurs propres.}
Les états propres $\ket{\{\theta_a\}}$, construits dans le cadre de la seconde quantification à partir de la solution du modèle de \gls{LL}, sont simultanément fonctions propres des opérateurs $\operator{Q}$, $\operator{P}$ et $\operator{H}$ :
...
\)
jouant ici un rôle analogue à celui d’un pseudo-moment associé dans le référentiel du laboratoire.
L'Hamiltonien relatif, \(\operator{\mathcal{H}}_{\text{rel}}\), correspond quant à lui à une particule de masse réduite \(\tilde{m} = \frac{1}{2}\) soumise à un potentiel delta centré en \(Y = 0\). %Son équation propre s’écrit :
Son état propre \(\tilde{\varphi}(Y)\) satisfait l’équation de Schrödinger suivante :
...
\paragraph{Forme symétrique de la fonction d'onde pour des bosons.}
Dans le référentiel du centre de masse, le système est le même que celui d'une particule de masse $\tilde{m}= \frac{1}{2}$.
Le système étant composé de particules bosoniques, on cherche une solution symétrique que l’on écrit sous la forme \eqref{chap.1.rapel.libre.4} :
\begin{eqnarray}
...
%et  la phase s'écrit (avec la définition usuelle de l’arctangente complexe,) 
et la phase \(\Phi\) s'écrit (avec la définition usuelle de l’arctangente complexe, ) 
\begin{eqnarray}
	\Phi(\tilde{\theta}) &=& 2 \arctan\left (\frac{1}{i} \frac{a+b}{a-b}\right),	\label{chap:1:dif.mod.2.part.1} 
\end{eqnarray}
%(avec la définition usuelle de l’arctangente complexe,) de sorte que l’on puisse réécrire l’ansatz~\eqref{eq:ansatz.boson} sous la forme trigonométrique symétrique.
...
%Dans le cas d'un couplage attractif ($g<0$) ,  cette énergie gouverne la décroissance exponentielle de la fonction d’onde dans la coordonnée relative : plus \(\tilde{\theta}\) est grand, plus l’état est localisé autour de \(Y=0\), signe d’une interaction attractive plus forte entre les deux bosons.
...
\begin{eqnarray}
	\tilde{\psi}_{outc} ( Y, t ) & \propto & -e^{i\Phi(\tilde{\theta})}e^{-i\frac{1}{2}\tilde{\theta} \vert Y\vert}\, e^{-i \tilde{\varepsilon}(\tilde{\theta})t} - e^{i\Phi(\tilde{\theta} + 2 \delta \tilde{\theta} )}e^{-i\frac{1}{2}\left(\tilde{\theta} + 2\delta \tilde{\theta} \right) \vert Y\vert} , e^{-i \tilde{\varepsilon}(\tilde{\theta} + 2 \delta \tilde{\theta}  )t}. %\tag{2}
\end{eqnarray}
...
\end{eqnarray}
où le {\bf déplacement collisionnel} $\Delta (\theta)$ est donné par la dérivée de la {\em phase de diffusion},
...
Dans ce cadre, le mouvement du centre de masse est indépendant de la collision, et se déplace à la vitesse de groupe
...
La figure~\ref{fig:collision_shift} illustre schématiquement la collision de deux particules et le décalage spatial $\Delta(\theta_1 - \theta_2)$ qui en résulte dans l’évolution semi-classique de leurs positions.

% \begin{figure}[h!]
%     \centering
% 	\includegraphics[width=0.5\textwidth , page = 1 ]{figures/01_LL_BA/Figures.pdf}
%     \caption{Schéma de collision entre deux quasi-particules et du décalage spatial $\Delta(\theta_1-\theta_2)$.}
%     \label{fig:collision_shift}
% \end{figure}
...
\begin{figure}[H]
		...
		\begin{scope}[shift={(0,0)}]
        	...
        	\node[above=-0.2cm of a , shift={(-2 , 0)}] {\fontsize{12pt}{14pt}\selectfont \bfseries a)};
          	...
          	\node[draw = none , rectangle , right = 0.5cm of a] (b)  {  
            	\centering
				\includegraphics[width=0.5\textwidth , page = 2 ]{figures/01_LL_BA/Figures.pdf}
          	};
	...
	\end{tikzpicture}
	\caption{Schéma de la collision entre deux quasi-particules illustrant le décalage spatial $\Delta(\theta_1-\theta_2)$.(a) description classique (sphère dure) ; (b) description quantique.}
	\label{fig:collision_shift}
\end{figure}
...
%Cela nous motive à multiplier l’ansatz dans le référentiel du centre de masse (Eq~\eqref{eq:ansatz.boson}) par un facteur de phase globale \(\exp(2iZ\overline{\theta})\) pour revenir à la représentation dans le laboratoire. On obtient alors l’expression de la fonction d’onde :
Pour revenir à la représentation observée dans le laboratoire, il est nécessaire de multiplier l’ansatz dans le référentiel du centre de masse (Eq.~\eqref{eq:ansatz.boson}) par un facteur de phase globale \(\exp(2iZ\overline{\theta})\). On obtient alors l’expression de la fonction d’onde :
\begin{eqnarray}\label{eq:I-1-10}
...
Le rapport de leurs amplitudes définit la {\bf phase de diffusion} $\Phi$ et la  {\bf matrice diffusion} $S$  à deux corps \eqref{chap:1:dif.mod.2.part.3}, reste inchangée, mais en voici une autre écriture :
...
Cette phase caractérise entièrement le processus de diffusion dans le modèle de \gls{LL} à deux particules.

\paragraph{Conditions périodiques et équations de Bethe pour deux bosons.}

Dans ce paragraphe, on écrit la fonction d’onde obtenue en \eqref{eq:I-1-10} sous la forme
\begin{eqnarray}\label{eq:I-1-10-1}
	\varphi_{\{\theta_1 < \theta_2\}}(z_1 , z_2) 
	&=&  a\, e^{ i z_1 \theta_1 + i z_2 \theta_2 } 
	+ b\, e^{ i z_1 \theta_2 + i z_2 \theta_1}, 
\end{eqnarray}
où $a$ et $b$ sont deux coefficients complexes.
...
On impose une condition de périodicité sur la fonction ~\eqref{eq:I-1-10-1} :
\(
...
\begin{eqnarray}\label{eq:I-1-10-2}
	a\,e^{i\theta_1 z_1 + i\theta_2 L} + b\,e^{i\theta_2 z_1 + i\theta_1 L}
	&=& a\,e^{i\theta_2 z_1} + b\,e^{i\theta_1 z_1}.
\end{eqnarray}

Cette relation, valable pour tout \( z_1 \in [0,L] \), fixe une contrainte sur le rapport \( b/a \). En utilisant l’expression de la phase de diffusion introduite en \eqref{chap:1:dif.mod.2.part.4} (valable pour $z_1<z_2$), on a :
\begin{eqnarray}\label{eq:I-1-10-3}
	-\frac{b}{a} = e^{i\Phi(\theta_1 - \theta_2)},
\end{eqnarray}
avec la fonction $\Phi$ donnée par l'équation \eqref{chap:1:dif.mod.2.part.2}.
...
$e^{i\theta_{2}z_{1}}$ indépendamment dans~\eqref{eq:I-1-10-2} , on obtient
\(
	e^{i\theta_{2}L}\;a = b, 
	\,
	e^{i\theta_{1}L}\;b = a,
\)
c’est‑à‑dire en utilisant \eqref{eq:I-1-10-3}, les \textbf{équations de \gls{BA} à deux particules} :
\begin{eqnarray}\label{eq:PC2}
	e^{i\theta_{1}L}\,e^{i\Phi(\theta_{1}-\theta_{2})} = -1,
	\qquad
	e^{i\theta_{2}L}\,e^{i\Phi(\theta_{2}-\theta_{1})} = -1.	
\end{eqnarray}
En prenant le logarithme, elles se réécrivent sous la forme usuelle :
\begin{equation}\label{eq:Bethe2}
	\theta_{1}L + \Phi(\theta_{1}-\theta_{2}) = 2\pi I_{1}, 
	\qquad
	\theta_{2}L + \Phi(\theta_{2}-\theta_{1}) = 2\pi I_{2},
\end{equation}
où $I_{1},I_{2}\in\mathbb{Z} + \tfrac{1}{2}$ sont des nombres demi-entiers. 

\subparagraph{Périodicité sur $z_{1}$.}  Le raisonnement symétrique conduit exactement aux mêmes égalités \eqref{eq:PC2} et \eqref{eq:Bethe2}.  Ces équations  constituent la quantification complète du gaz de \acrlong{LL} à deux bosons sur un cercle de longueur $L$ et seront le point de départ pour l’étude de l’état fondamental et des excitations.

\medskip

%Au debut du chapitre on a mis en équation les confition au bord pertiodique pour une une particule qui est bien connue pour maintent en comparant avec les equation de Bethe à deux particule \eqref{eq:PC2} et \eqref{eq:Bethe2} pour voir metre en evidende que la fonction $\Phi$ définie en \eqref{chap:1:dif.mod.2.part.2} encode les interaction 

En comparaison avec le cas d’une seule particule (où la condition aux bords périodiques impose simplement $\theta L = 2\pi n$ ~\eqref{chap.1.rapel.libre.5}), on voit que les équations de \gls{BA} à deux particules \eqref{eq:PC2}--\eqref{eq:Bethe2} incorporent la fonction $\Phi$, qui encode explicitement l’\textbf{interaction entre particules}.
...
Ainsi , dans le cas à deux corps, les équations de \gls{BA} quantifient les valeurs possibles des rapidités $\theta_1$ et $\theta_2$ en fonction de la taille du système $L$ et de l’interaction entre particules via la phase de diffusion $\Phi(\theta_1 - \theta_2)$. Chaque solution $(\theta_1, \theta_2)$ correspond à un état propre du système, où les deux quasi-particules se propagent avec des vitesses $\theta_1$ et $\theta_2$, tout en incorporant les effets d’interaction.
Dans l’image de Bethe, les états propres sont des configurations stables de $N$ quasi-particules de rapidités $\theta_1,\dots,\theta_N$, dont les valeurs sont quantifiées par les conditions d’interactions imposées par les équations de \gls{BA}.
...
Sans en faire ici la démonstration, donnons une forme généralisée de la fonction propre à $N$ particules (Eq~ \eqref{chap.1:eq.rapel.fonction.propre.N}), qui prolonge naturellement l’expression obtenue pour le cas à deux corps (Eq~ \eqref{eq:I-1-10} et Annex \ref{annex:Fourier}).
Dans le domaine $z_1 < z_2 < \cdots < z_N$, la fonction d’onde pour un état de Bethe à $N$ particules s’écrit  \cite{Korepin_1997,Gaudin_2014,LL_1963_1,Franchini_2016,Wouters2015}) :
\begin{eqnarray}
...
En imposant les conditions de périodicité sur la fonction d’onde de type Bethe~\eqref{eq:I-2-17}, on généralise l'équation\eqref{eq:PC2} pour $N$ particules. On obtient que chaque moment $\theta_a$ doit satisfaire l’équation :
\begin{equation}
...
\end{equation}
où les $I_a$ sont des nombres quantiques. Ce sont des entiers $I_a \in \mathbb{Z}$ si $N$ est impair, et des demi-entiers $I_a \in \mathbb{Z} + \tfrac{1}{2}$ si $N$ est pair.

\medskip

Dans la configuration d’état fondamental (ou de type “mer de Fermi”), ces nombres sont pris de manière symétrique autour de zéro (voir Fig \ref{fig:fermi_excitations}(a) ):
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.9\textwidth , page = 11 ]{figures/01_LL_BA/Figures.pdf}
    \caption{
    Représentation schématique de la mer de Fermi pour un système de $N$ particules :  
    (a) distribution occupée à l’état fondamental, analogue à une mer de Fermi idéale ;  
    (b) modification de cette distribution sous l’effet d’une excitation élémentaire de type~I ;  
    (c) modification correspondante en présence d’une excitation élémentaire de type~II.
    }
    \label{fig:fermi_excitations}
\end{figure}
...
Tous les nombres $I_a$ sont distincts. Cette propriété établit un lien direct avec un gaz de \textbf{fermions libres} en une dimension : bien que les particules du modèle de \gls{LL} soient bosoniques, la structure des équations de \gls{BA} impose une exclusion effective au niveau des nombres quantiques. Pour cette raison, on parle de \textbf{nombres quantiques fermioniques}.  
L’équation de \gls{BA} \eqref{chap:1:eq:EBA} définit ainsi une bijection entre une configuration de rapidité $\{ \theta_a \}$ (\gls{BA}) et un ensemble de nombres quantiques $\{ I_a \}$ (\gls{FBA}) \cite{Girardeau_1960}.


\medskip

En particulier, dans la limite d’\textbf{interaction forte} (\ie $g \to \infty$ c’est-à-dire si $\Phi = 0$), les équations \eqref{chap:1:eq:EBA} se réduisent à $L \theta_a = 2\pi I_a$, soit $\theta_a = 2\pi I_a / L$. Cela correspond exactement aux quantifications de l'impulsion pour des {\bf fermions libres sans spin}, dans une boîte de taille $L$, avec conditions aux bords périodiques.
...
Les équations de \gls{BA}~\eqref{chap:1:eq:EBA} représentent une {\em quantification des pseudo‑impulsions $\theta_a$} des particules en interaction, résultant d’un {\em interféromètre multi‑corps sur le cercle} : chaque particule accumule une phase $e^{i \theta_a L}$ due au mouvement libre, ainsi que des phases de diffusion lorsqu’elle croise les autres.

Ce système d’équations non linéaires couple toutes les rapidités $\theta_a$ entre elles via la fonction de phase de diffusion $\Phi(\theta_a - \theta_b)$, qui encode les interactions à deux corps. La résolution de ces équations permet de déterminer les valeurs permises des rapidités $\theta_a$, et donc les états propres du système.
Ce système d'équations détermine les états propres du système de \acrlong{LL}  en volume fini, et joue un rôle fondamental dans la description exacte de ses propriétés thermodynamiques et dynamiques.
...
Considérons désormais le système à température nulle. L’état fondamental dans le secteur à nombre de particules fixé correspond à la configuration d’énergie minimale parmi les solutions des équations de \gls{BA} \eqref{chap:1:eq:EBA}.

Dans la limite thermodynamique $(\underset{\mathrm{\tiny therm}}{\lim} \equiv \underset{\underset{ N/L = \mathrm{cnst}}{L,N \to \infty,\infty}}{\lim} )$ , les valeurs de \( \theta_a \) deviennent quasi-continues, avec un espacement \( \theta_{a+1} - \theta_a = \mathcal{O}(1/L) \), et se condensent dans un intervalle symétrique autour de zéro $[-\theta_{\mbox{\tiny max}} , \theta_{\mbox{\tiny max}} ]$. En supposant que l'ordre \( I_a \geq I_b  \) implique que \(\theta_a \geq \theta_b \), cet intervalle constitue ce qu'on appelle la {\em mer de Fermi} (ou sphère de Fermi en dimension un).

\medskip

\paragraph{Distribution de rapidité $\rho(\theta)$.}

Nous introduisons la {\bf distribution de rapidité macroscopique} par unité de longueur, notée \( \rho(\theta) \), définie par
\begin{eqnarray}
...
\end{eqnarray}
Avec les rapidités $\theta_a$ ordonnées \ie $\theta_1 < \theta_2 < \cdots < \theta_N$. 

Pour des intervalles $[\theta_a , \theta_a + \delta \theta_a]$ assez petits, on a $\delta \theta_a = \theta_{a+1} - \theta_a$. Cela correspond à $\delta N (\theta_a) = 1 $ quasi-particule, car une seule particule occupe l’intervalle entre deux rapidités consécutives. Dans la limite thermodynamique la dernière équation \eqref{chap.1:eq.dN.1} se réécrit comme :
\begin{eqnarray}
	\rho(\theta_a) & = & \underset{\mathrm{\tiny therm}}{\lim} \frac{1}{L} \cdot \frac{1}{\theta_{a+1} - \theta_a} > 0.
\end{eqnarray}
On peut aussi écrire la distribution de rapidité sous la forme :
\begin{eqnarray}\label{chap.1:eq.rho.111111}
	\rho(\theta) & = & \underset{\mathrm{\tiny therm}}{\lim} \frac{1}{L} \sum_{a = 1}^N \delta ( \theta - \theta_a ),	
\end{eqnarray}
où $\delta(\cdot)$ est la distribution delta de Dirac.

\medskip

\paragraph{Densité d’états $\rho_s(\theta)$.}

Intéressons-nous maintenant à la {\bf densité d’états}  \( \rho_s(\theta) \), définie par
\begin{eqnarray}
	 \rho_s(\theta_a) &\doteq & \frac{1}{L} \lim_{\text{\tiny therm}} \frac{|I_{a+1} - I_a|}{|\theta_{a+1} - \theta_a|},
\end{eqnarray}
soit en notant la fonction \( I(\theta_a) = I_a \), le passage discret-continu permet d'écrire :
\begin{eqnarray}
	2\pi \rho_s(\theta_a) &=&  \frac{2\pi}{L} \frac{\partial I}{\partial \theta}(\theta_a).
\end{eqnarray}
L’application des équations de \gls{BA} sous forme logarithmique \eqref{chap:1:eq:EBA} conduit alors à
\begin{eqnarray}\label{chap1:eq:rho.s.1}
	2\pi \rho_s(\theta_a) = 1 + \frac{1}{L} \sum_{b = 1}^N \Delta(\theta_a - \theta_b),
\end{eqnarray}
ce qui relie \( \rho_s \) au déplacement collisionel \( \Delta \) défini dans l'équation  \eqref{eq:I-1-16}.


\begin{figure}[h]
    \centering
    \includegraphics[width=1.0\textwidth , page = 10 ]{figures/01_LL_BA/Figures.pdf}
    \caption{
	Illustration de la relation entre la densité d’états à l’état fondamental,
\(\rho(\theta)=\rho_s(\theta)\), et les nombres quantiques fermioniques \(I_a\).
Les rapidités contenues dans l’intervalle \([\theta,\theta+\delta\theta]\)
correspondent à un ensemble de nombres quantiques fermioniques situés dans
\([I,I+\delta I]\).
À l’état fondamental, la densité d’états se déduit de la relation
\(
\delta I \simeq L\,\rho(\theta)\,\delta\theta .
\)
    }
    \label{fig:rho_s.I}
\end{figure}
...
\)
donne le nombre total de particules dans le système. Le passage de la somme discrète à l'intégrale dans le second membre de l'équation de \gls{BA} s'effectue en utilisant la correspondance:
\begin{eqnarray}
...
\end{eqnarray}
Ainsi, l'équation pour la densité d'états \eqref{chap1:eq:rho.s.1} devient (Fig ~\ref{fig:rho_s.I}):
...
\end{eqnarray}
et, comme \( \rho = \rho_s \) dans  l'état fondamental, on obtient l’équation linéaire intégrale satisfaite par la distribution de rapidités dans cet état :
...
À partir de l’état fondamental dans le régime d’interaction forte, les excitations élémentaires du modèle de \gls{LL} ont été classifiées par Lieb en deux types distincts~\cite{LL_1963_2}. Ces excitations peuvent être comprises comme des perturbations de la mer de Fermi formée par les quasi-particules.

\begin{itemize}[label = $\bullet$]
    \item \textbf{Excitations de type I :} Il s'agit de remplacer une quasi-particule avec un nombre fermionique au bord de Fermi $I_{N}$ (ou $I_{-N}$) par une quasi-particule avec un nombre fermionique $I'> I_N$ (resp.$I' <I_{-N}$), c’est-à-dire au-delà du bord de la mer de Fermi (voir Fig \ref{fig:fermi_excitations}(b)). Ces excitations sont analogues à des excitations de particules libres et, dans la limite des faibles interactions ($g \to 0$), leur relation de dispersion reproduite celle prédite par la théorie de Bogoliubov.

    \item \textbf{Excitations de type II :} Ces excitations sont de type particule-trou. Elles sont obtenues en remplaçant une quasi-particule de la mer de Fermi avec un nombre fermionique $I_a$ (\ie, en créant un trou dans l’état fondamental) et de le remplacer par une quasi-particule avec $I' = I_{N+1} = N/2$ ou $I' = I_{N-1} = -N/2$ et en réarrangeant les autres nombres fermionique (voir Fig \ref{fig:fermi_excitations}(c)). Cela correspond à une excitation interne  pseudo-mer de Fermi, avec conservation du nombre de particules. Pour de petits $I'$, la dispersion est linéaire, correspondant à des modes phononiques. Dans la limite d’interaction faible \( g \to 0 \), ces excitations peuvent être interprétées comme des solitons sombres~\cite{Ishikawa1980SolitonsIA,Karpiuk_2015}.
\end{itemize}

Ces deux types d’excitations définissent ensemble le spectre complet du modèle de \gls{LL} et permettent d’accéder aux propriétés dynamiques du système, telles que les fonctions de réponse ou la structure du spectre d’énergie.

\paragraph{Équation de Bethe continue.}

Hors de l’état fondamental, la notion de « mer de Fermi » n’est plus bien définie, et les équations~\eqref{chap.1.rho.2} et \eqref{chap.1.rho.3} ne sont donc plus valides (en particulier, on n’a plus $\rho = \rho_s$). 

%Plus généralement, non nécessairement dans l'état fondamantal ( \ie hors et à l'état fondamanttal), les équations de Bethe~\eqref{chap.1.rho.s.2} se condensent, à la limite thermodynamique, en une équation intégrale reliant les densités de rapidité :

\smallskip 

De manière plus générale, que l’on soit ou non dans l’état fondamental (\ie{} aussi bien dans l’état fondamental qu’à l’extérieur de celui-ci), les équations de Bethe~\eqref{chap.1.rho.s.2} se condensent, dans la limite thermodynamique, en une équation intégrale reliant les densités de rapidité :


\begin{equation}\label{eq:TBA-rhos}
	2\pi \rho_s(\theta) \;=\; 1 \;+\; \bigl(\Delta \star \rho\bigr)(\theta),
\end{equation}
où le symbole $\star$ désigne la \emph{convolution} :
\begin{eqnarray*}
	\bigl[\Delta \star \rho\bigr](\theta) & = &  \int_{-\infty}^{\infty} d\theta'\, \Delta(\theta - \theta')\, \rho(\theta').
\end{eqnarray*}

On introduit alors la \textbf{densité de trous} $\rho_h(\theta)$ définie par
\begin{equation}
	\rho_s(\theta) \;=\; \rho(\theta) + \rho_h(\theta),
\end{equation}
ainsi que la \emph{fonction d’occupation} (ou facteur d’occupation de type Fermi–Dirac)
\begin{equation}\label{eq:TBA-nu}
...
\begin{equation}\label{eq:dressing}
	f^{\mathrm{dr}}_{[\nu]}(\theta) 
	\;=\; f(\theta) \;+\; \frac{1}{2\pi}\,\bigl(\Delta \star [\nu\, f^{\mathrm{dr}}]\bigr)(\theta).
\end{equation}

Ici, $\nu(\theta)$ est le \textbf{facteur d’occupation}, et $\Delta/2\pi$ est le \textbf{noyau de diffusion} caractéristique du modèle considéré.  

\medskip

\subparagraph{Interprétation physique}

Le dressing incorpore à tous ordres les effets de rétrodiffusion entre quasi-particules. Il encode ainsi les corrections d’interaction aux grandeurs physiques initiales $f(\theta)$. L’opération de \emph{dressing} joue un rôle central dans la description thermodynamique et dynamique des systèmes intégrables : elle permet d’exprimer les grandeurs physiques effectives (charges, courants, vitesses de groupe, etc.) en termes de leurs versions habillées par les interactions. %Dans le modèle de Lieb–Liniger, cette opération permet de déterminer : l’énergie habillée $\varepsilon^{\mathrm{dr}}(\theta)$ , l’impulsion habillée $p^{\mathrm{dr}}(\theta)$ , les susceptibilités thermodynamiques (cf. section~\ref{chap:GGE}).
%
%{\color{blue}
...
\end{eqnarray}
ce qui n’est autre que la relation~\eqref{eq:TBA-rhos}.
...
Dans ce chapitre, nous avons introduit le modèle de \acrfull{LL}, qui décrit un gaz de bosons unidimensionnel soumis à une interaction de contact. Dans ce cadre, les grandeurs conservées naturelles ne sont pas seulement le nombre total de particules $\operator{Q}$ et la quantité de mouvement totale $\operator{P}$, mais également l’énergie associée à l’Hamiltonien \gls{LL}, $\operator{H}$, (plutôt qu’à la simple énergie cinétique $\operator{K}$). Ces charges conservées correspondent respectivement aux moments d’ordre $0$, $1$ et $2$ en rapidité. Comme nous le verrons au chapitre suivant, des charges d’ordre supérieur interviennent dans la description des états en équilibre, notamment via l'Ensemble de Gibbs Généralisé (\gls{GGE}).

\medskip

L’imposition de conditions aux bords périodiques conduit naturellement aux équations de \acrfull{BA}, voir \eqref{chap:1:eq:EBA}. Dans la limite thermodynamique, celles-ci se condensent en une équation intégrale reliant la densité d’états $\rho_s(\theta)$ à la distribution de rapidités $\rho(\theta)$, cf. \eqref{eq:TBA-rhos}. Le \glsentrylong{BA} établit ainsi le lien fondamental entre ces deux grandeurs et constitue la base de la description thermodynamique du modèle.

\medskip

Le chapitre suivant sera consacré à l’introduction du \gls{GGE} et à l’écriture explicite du formalisme de Bethe Ansatz thermodynamique  (\acrfull{TBA}), qui fournit un cadre puissant pour décrire les propriétés thermodynamiques et hors équilibre des systèmes intégrables.
}
