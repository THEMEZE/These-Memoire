...
Considérons un atome immobile possédant un état fondamental \( \ket{g} \) et une série d’états excités \( \ket{e_i} \), d’énergies respectives \( \hbar\omega_g \) et \( \hbar\omega_{e_i} \). Le champ laser \eqref{chap7:eq.elec.clas.1} est quantifié dans un volume \( V \), polarisé selon \( \vec{u} \),  et d'expression \cite{CohenTannoudji1988}
%\begin{eqnarray}
...
\begin{itemize}[label = $\bullet$]
	...
	\item Le {\bf taux de diffusion spontané}  au point $x^\ast$ défini par $U_{dip}(x^\ast) = U_{dip}(0)/2$ doit rester inférieur à une valeur seuil pour limiter l’échauffement et les pertes :
		...
		\end{equation}
		Ce critère conduit, en utilisant le fait que le potentiel $U_{dip}^{(min)}(\vec{r})$ et le taux de diffusion spontanée $\Gamma_{sp}(\vec{r})$ soit proportionnelle à l’intensité $I(\vec{r})$, à la borne stricte suivante :
...
Les conditions des Eqs.~\eqref{eq:7.49} et \eqref{eq:7.50} impliquent que, pour un point particulier \(\vec{r}^*\), le rapport entre la profondeur du piège et la diffusion photonique satisfait
\[
...
Les conditions des Eqs.~\eqref{eq:7.49} et \eqref{eq:7.50} impliquent alors, en utilisant les expressions \eqref{eq:7.46} et \eqref{eq:7.47} pour le potentiel dipolaire et le taux de diffusion de photons, que la puissance laser doit satisfaire
\begin{equation}
...
\begin{itemize}[label=$\bullet$]
    ...
    \item \textbf{Mode spatial en sortie :} la sortie du \gls{TA} reste monomode, mais le profil spatial est fortement déformé et s’éloigne d’un mode TEM$_{00}$ parfait. Un trou de filtrage ou une \textbf{trou de filtrage} de $50~\mu$m (à ajuster selon le profil observé) peut être utilisé pour filtrer et améliorer la qualité spatiale du faisceau.
\end{itemize}
...
\begin{itemize}[label=$\triangleright$]
    ...
    \item \textbf{Modulation de puissance :} un modulateur acousto-optique \gls{AOM} est inséré juste avant la fibre optique, permettant de couper la lumière à la demande (switch rapide) ou de moduler dynamiquement l’intensité laser. Cela permet un contrôle fin de l’illumination au niveau des atomes.
    
    \item \textbf{Mesure de puissance :} une photodiode ou un puissance-mètre placé après l’AOM permet de mesurer précisément la puissance en fonction du courant injecté dans le \gls{TA}, et de corriger les effets d’instabilité si nécessaire.
