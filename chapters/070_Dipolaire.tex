\chapter{Mise en place d'un confinement longitudinale dipolaire}\label{chap:dipolaire}
\minitoc
...
\begin{itemize}[label = $\bullet$]
	\item Le {\bf potentiel dipolaire} au niveau des atomes, $U_{dip}(0)$, doit être supérieur à une valeur minimale $U_{dip}^{(min)}$, fixée à :
		\begin{equation}\label{eq:7.50}
			U_{dip}(0)  \geq U_{dip}^{(min)} \equiv k_B \times 1 \mu K.	
		\end{equation}
	...
	\item Le {\bf taux de diffusion spontané}  au point $x^\ast$ défini par $U_{dip}(x^\ast) = U_{dip}(0)/2$ doit rester inférieur à une valeur seuil pour limiter l’échauffement et les pertes :
		\begin{equation}\label{eq:7.51}
			\Gamma_{sp}(x^\ast) \leq \Gamma_{sp}^{(max)} \equiv 2\pi \times 1~s^{-1}.	
		\end{equation}
...
\]
En utilisant les équations ~\eqref{eq:7.47} et \eqref{eq:7.48}, cette condition se réécrit explicitement sous la forme
%\begin{equation}
...
avec
\begin{align}
P_{\rm min}(\lambda) &= U_{\rm dip}^{(min)}  \frac{\pi w_0^2}{2} \frac{8 I_{\rm sat}}{\hbar \Gamma^2} 
\left( \frac{1}{3 \Delta_1} + \frac{2}{3 \Delta_2} \right)^{-1}, \\
P_{\rm max}(\lambda) &= \Gamma_{\rm sc}^{(max)}  \pi w_0^2 \frac{8 I_{\rm sat}}{\Gamma^3} 
