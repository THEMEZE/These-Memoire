...
Dans le chapitre précédent \ref{chap:bipartite}, notre objectif était de préparer un système aussi homogène que possible sur une large région spatiale. Pour cela, nous avions recours à un potentiel longitudinal quartique, qui permettait d’obtenir un gaz quasi-homogène tout en conservant une occupation suffisante. Toutefois, dans la perspective d’améliorer encore cette homogénéité et d’optimiser la mesure des fluctuations présentée au chapitre \ref{chap:Fluctu}, il devient nécessaire de disposer d’un contrôle plus flexible et plus local du paysage de potentiel. C’est dans ce cadre que nous introduisons ici une implémentation d’un potentiel dipolaire repoussoir, capable de créer des barrières nettes et ajustables. Couplé à un \gls{DMD} et à un \gls{AOM}, ce schéma ouvre la possibilité de générer des géométries de piégeage plus sophistiquées et adaptées à la préparation d’états quasi-uniformes ou à l’ingénierie de potentiels plus exotiques.
...
\section{Transformation de jauge et simplification de l'Hamiltonien}

%(cf. Complément HIII du tome 1 de Cohen-Tannoudji)\\

%\subparagraph{Remarques préliminaires et extensions}
%
%Avant d’appliquer la transformation de jauge et de simplifier le Hamiltonien, il est utile de souligner certains points 
%qui conditionnent la conservation des quantités physiques et la cohérence du formalisme :
%
%\begin{itemize}[label = $\bullet$]
%    \item La conservation de la quantité de mouvement totale $P$ n’est assurée que si le système est parfaitement invariant par translation. En présence d’un \textbf{potentiel externe} $V(x)$, ou lorsque des conditions aux limites (bords durs, pièges) brisent cette symétrie, $\operator{P}$ n’est plus une constante du mouvement.
%    \item Si un \textbf{champ électromagnétique} est appliqué, la jauge introduit le potentiel vecteur $\vec{A}$, et l’élan cinétique devient $\vec{\Pi} = \vec{p} - q \vec{A}$. L’expression de $\vec{P}$ doit alors être redéfinie pour rester cohérente  avec la dynamique.
%    \item Dans le cas de \textbf{potentiels dipolaires} ou d’interactions à longue portée, les charges conservées 
%    peuvent être modifiées. Il est alors nécessaire d’inclure des termes supplémentaires afin de respecter les 
%    symétries fondamentales.
%\end{itemize}
%
%Ces précisions fournissent le cadre conceptuel dans lequel la transformation de jauge garde toute sa cohérence et servent d’introduction naturelle à l’étude des systèmes avec interactions complexes et charges conservées d’ordre supérieur.
...
La transformation de jauge que nous avons appliquée permet de travailler dans un cadre avec lequel le{ \bf  potentiel vecteur} est nul. Dans cette jauge particulière, l'Hamiltonien du système est considérablement simplifié, car le couplage au champ électromagnétique ne se fait plus par le terme de couplage minimal \((\operatorvec{P} - q \operatorvec{A})^2\), mais uniquement à travers un {\bf potentiel scalaire effectif}.
...
Avant d'effectuer le calcul du potentiel dipolaire, donnons une image classique.

\smallskip

Un atome neutre placé dans un champ électrique $\operatorvec{E}_\perp$ développe un moment dipolaire induit $\operatorvec{D}(t) = \alpha \operatorvec{E}_\perp$. L'énergie potentielle d'interaction, moyennée temporellement sur une période optique, est alors $-\alpha \Vert\operatorvec{E}_\perp\Vert^2/2$ . Si le champ varie lentement devant la dynamique interne de l’atome, le moment dipolaire reste aligné avec la composante transverse du champ, $\operatorvec{E}_\perp(t)$, et l’énergie potentielle d’interaction s’écrit 
\(
...
On suppose que le champ laser est {\bf désaccordé par rapport à la résonance} et que le désaccord
\(
...
est {\bf grand devant la fréquence de Rabi}, qui caractérise le couplage dipôle–champ dont l'expression est \cite{CohenTannoudji1988, grimm1999opticaldipoletrapsneutral, Schlosser2001, Darquie2006} :  
\begin{eqnarray}
...
Dans le cas d’une transition dipolaire électrique de type S–P, il peut être calculé à partir de l’électrodynamique quantique selon la formule \cite{grimm1999opticaldipoletrapsneutral} :
\begin{equation}
...
\item \textbf{Ordre 2} — transitions virtuelles :  nous verrons dans la section \ref{sec.7.3.1} que l'effet du potentiel à l'ordre 2 est
...
Dans l’approximation de l’ordre tournant (ou approximation séculière, RWA), ce terme est négligé car il est fortement décalé en énergie (\(\omega + \omega_{\scriptscriptstyle g\rightarrow e} \gg \omega - \omega_{\scriptscriptstyle g\rightarrow e}\), ou \(\omega \gg \Delta \) ) .
...
En utilisant l'expression de la fréquence de \emph{Rabi} \eqref{chap7:eq.Omega.1}, la correction d’énergie à l’ordre 2 \eqref{chap7:eq.E_g(2).1}, que l’on notera \emph{potentiel dipolaire} (\ie \(U_{\mathrm{dip}}(\vec{r}) \equiv \delta E_g^{(2)}\)), s’écrit pour \( \omega \gg \Delta \) :
...
Ce potentiel permet de décrire le confinement des atomes dans des régions où \( I(\vec{r}) \) est élevé (ou faible, selon le signe de \( \Delta \)), formant ainsi des barrières optiques contrôlables avec une résolution posiiblement sub-micronique.


\subparagraph{Conditions de validité.}

Les expressions~\eqref{chap7:eq.dip.1} et \eqref{chap7:eq.dip.2}  pour le potentiel dipolaire sont obtenues sous les hypothèses suivantes, qui assurent la validité du modèle à deux niveaux et du traitement perturbatif :
...
\begin{itemize}[label = $\bullet$]
  ...
  \item \textbf{Approximation du sous-espace résonant} : le champ est quasi-résonant avec une seule transition atomique, ce qui implique que le désaccord $\Delta$ reste petit par rapport à l’écart énergétique entre l’état excité considéré et les autres états excités de l’atome. Cette condition permet de réduire le système à un modèle à deux niveaux et de négliger les autres transitions dipolaires.
  
  \item \textbf{Régime de moyen désaccord en fréquence (large détuning)} : lorsque $|\Delta| \gg \Gamma$, c’est-à-dire lorsque le désaccord est grand devant la largeur naturelle de la transition, les excitations réelles vers l’état excité sont fortement supprimées. L’interaction lumière-matière peut alors être traitée en perturbation du second ordre : l’atome reste majoritairement dans son état fondamental, et l’effet du champ lumineux se manifeste sous la forme d’un potentiel effectif induit par des couplages virtuels.\footnote{Il faut aussi $\omega \gg \Delta$ pour justifier l'approximation de l'ordre tournant (RWA).}
  
  \item \textbf{Régime de faible saturation} : on suppose que la fréquence de \emph{Rabi} $\Omega$ est beaucoup plus faible que le désaccord, i.e. $\Omega \ll |\Delta|$. Cette condition garantit que la population de l’état excité reste négligeable, ce qui justifie l’approximation adiabatique sur l’état fondamental.
\end{itemize}
...
Le potentiel dipolaire \( U_{\mathrm{dip}}(\vec{r}) \), dépendant de la position via l’intensité du champ laser, permet de confiner des atomes neutres en créant des paysages de potentiel contrôlés avec des régions de potentiel attractif ou répulsif.

\smallskip

Dans le cas d’un désaccord bleu (\( \Delta > 0 \)), les atomes sont repoussés des zones de forte intensité. On peut alors façonner des \textit{barrières de potentiel} en structurant l’intensité lumineuse, par exemple à l’aide de phénomènes ' interférents ou de diffraction. Cela permet de créer des cavités, des guides ou des réseaux où les atomes sont confinés entre les zones lumineuses, sans nécessairement focaliser de faisceau.

\smallskip

En revanche, pour un désaccord rouge (\( \Delta < 0 \)), les atomes sont attirés vers les régions de forte intensité. En focalisant un faisceau laser, on peut créer des \textit{puits de potentiel} où les atomes sont piégés au centre du faisceau. Ce principe est largement utilisé dans les expériences de refroidissement et de confinement d’atomes ultrafroids, permettant de manipuler et d’étudier des gaz quantiques avec une grande précision.
...
\subsection{Atomes multiniveaux et origine du potentiel dipolaire dans le formalisme quantique}\label{sec.7.3.1}

Le modèle à deux niveaux est suffisant pour introduire le concept de piégeage dipolaire, mais il reste limité pour décrire les détails réels d'un atome comme le \({}^{87}\mathrm{Rb}\), qui possède une structure hyperfine et fine complexe. Dans ce contexte, le potentiel dipolaire dépend du sous-niveau quantique occupé par l’atome, et le traitement doit être généralisé, si \( \Delta \) n'est pas grand devant les séparations énergétiques entre les différents niveaux excités fines.

\subsubsection{Description quantique de l’atome et du champ laser}

\paragraph{Quantification de champ laser.}

Considérons un atome immobile possédant un état fondamental \( \ket{g} \) et une série d’états excités \( \ket{e_i} \), d’énergies respectives \( \hbar\omega_g \) et \( \hbar\omega_{e_i} \). Le champ laser \eqref{chap7:eq.elec.clas.1} est quantifié dans un volume \( V \); on le suppose polarisé selon \( \vec{u} \);  son expression \cite{CohenTannoudji1988} est alors :
%\begin{eqnarray}
...
Donc, dans l’approximation cohérente et intense (\(\langle N \rangle \gg 1\)), le champ quantique peut être remplacé par un {\bf champ classique}  :
\begin{equation}
...
Pour décrire complètement l'interaction entre l'atome et le champ laser, il est utile de présenter l'Hamiltonien total du système, qui se compose de trois parties principales correspondant à l'atome, au champ et à leur interaction~\cite{CohenTannoudji1988} :
\begin{eqnarray}
\operator{H} = \operator{H}_{\mathrm{at}} + \operator{H}_{\mathrm{L}} + \operator{H}_{\mathrm{int}}.	
\end{eqnarray}
Nous travaillons ici dans le formalisme {\bf atome + champ quantifié}. Ce formalisme est aussi appelé {\bf formalisme de l’atome habillé}. Ainsi :
...
\begin{itemize}[label = $\bullet$]
	...
	\item $\operator{H}_{\mathrm{int}} = - \operatorvec{D} \cdot \operatorvec{E}_\perp$, l’interaction \textbf{dipolaire quantifiée} entre l’atome et le champ électrique transverse $\operatorvec{E}_\perp$ \eqref{chap7:eq.elec.quat.1},  qu'on peut écrire sous la forme:
		\begin{eqnarray}
			\operator{H}_{\mathrm{int}} 
			= \sqrt{\frac{\hbar \omega}{2\varepsilon_0 V}} \, (\operatorvec{D} \cdot \vec{u}) \, (\operator{a} + \operator{a}^\dagger).
		\end{eqnarray}
		Dans l’\textit{approximation des grandes longueurs d’onde} ($\lambda \gg \langle \vec{r} \rangle$, où $\vec{r}$ désigne la coordonnée électronique relative au noyau), cet Hamiltonien prend la forme de l’\textbf{Hamiltonien dipolaire électrique}~\cite{CohenTannoudji1988, grimm1999opticaldipoletrapsneutral}.  L’opérateur de moment dipolaire atomique $\operatorvec{D}$ se décompose comme
		\(
		\operatorvec{D} 
		...
		Cette écriture traduit le fait que seules les transitions dipolaires électriques permises par les règles de sélection (changement de parité et conservation du moment angulaire) participent au couplage avec le champ quantifié dans le cadre de l'approximation dipolaire.
...
\begin{itemize}[label = $\bullet$]
  ...
  \end{eqnarray}
	...
	\end{eqnarray}
	qui permet d’exprimer l’amplitude du champ électrique \( \mathcal{E} \) en fonction du nombre moyen de photons \( \langle N \rangle \) dans le champ laser.
\end{itemize}
...
% \( \operator{H}_{\text{int}} = - \operatorvec{D} \cdot \operatorvec{E}_\perp \) 
% couple les états \( \ket{g, N+1} \) et \( \ket{e_i, N} \) via l’absorption ou l'émission d’un photon, 
...
L’interaction dipolaire 
\(
    H_{\text{int}} = - \vec{D} \cdot \vec{E}_\perp
\)
couple spécifiquement les états \( \ket{g, N+1} \) et \( \ket{e_i, N} \), 
correspondant respectivement à l’absorption et à l’émission d’un photon. 
Dans l’approximation séculaire, seul ce couplage résonant est conservé : 
tous les autres termes — en particulier le couplage 
\( \ket{g, N} \leftrightarrow \ket{e_i, N+1} \) — oscillent rapidement dans l’interaction 
et sont ainsi négligeables devant les termes quasi résonants.\footnote{
Dans l’approximation séculaire, on ne conserve que les termes de l’interaction dont la 
fréquence d’oscillation est proche de zéro dans l’image d’interaction.  
Les énergies 
\( E_{g,N} = N \hbar \omega \) et 
\( E_{e_i,N} = \hbar\omega_{e_i\rightarrow g} + N\hbar\omega \)
montrent que la différence d’énergie entre 
\( \ket{g,N+1} \) et \( \ket{e_i,N} \) est 
proche de la résonance atomique.  
En revanche, les termes tels que 
\( \ket{g,N} \leftrightarrow \ket{e_i,N+1} \) 
contiennent un facteur oscillant 
\( e^{\pm i(\omega + \omega_{e_i\rightarrow g})t} \), 
loin de la résonance, et moyennent à zéro. 
On ne garde donc que le seul bloc résonant 
\( \ket{g,N+1} \leftrightarrow \ket{e_i,N} \).
}
...
\paragraph{Annulation de la correction d’énergie au premier ordre. \cite{Le_Kien_2013,grimm1999opticaldipoletrapsneutral} }
...
\begin{itemize}[label = $\bullet$] 
	...
	\( \operator{H}_{\text{int}} = - \operatorvec{D} \cdot \operatorvec{E}_\perp \) 
	est de parité impaire\footnote{
Un opérateur \(O\) est dit \emph{de parité impaire} si son comportement 
sous l’opérateur de parité \(P : \vec{r} \mapsto -\vec{r}\) vérifie 
\( P O P^{-1} = - O \).  
Le moment dipolaire \( \vec{D} = -e\,\vec{r} \) est ainsi un vecteur 
qui change de signe sous \( \vec{r} \to -\vec{r} \).  
Les états électroniques atomiques \( \ket{g}, \ket{e_i}, \dots \) 
sont en revanche des fonctions propres de la parité : 
\( P\ket{\psi} = \pm \ket{\psi} \).  
Dès lors, l’élément de matrice 
\( \bra{g} \vec{D} \ket{g} \) ou \( \bra{e_i} \vec{D} \ket{e_i} \) 
est nul par symétrie, alors que 
\( \bra{e_i} \vec{D} \ket{g} \neq 0 \) 
si et seulement si les parités de \( \ket{g} \) et \( \ket{e_i} \) sont opposées.
}, alors que les états électroniques 
...
\paragraph{Approximation pour un couplage quasi-résonant.}

Dans le cadre du piégeage dipolaire, nous considérons que le champ laser est quasi-résonant avec une seule transition atomique. 
Autrement dit, le désaccord \(\Delta = \omega - \omega_{g \rightarrow e}\) reste petit devant la fréquence de la transition atomique :
\(
|\Delta| \ll \omega_{g \rightarrow e}.
\)

Dans ce régime, le deuxième terme du membre de droite des équations \eqref{eq.7.34} devient négligeable, et le potentiel dipolaire se simplifie alors en :
\begin{eqnarray}\label{eq:potentiel_dipolaire_simplifie}
\delta E_g^{(2)} \simeq \sum_{e_i} \frac{\left| \bra{e,\,N-1} \operator{H}_{\mathrm{int}} \ket{g,\,N} \right|^2}
{\hbar \Delta} .
\end{eqnarray}

%Cette approximation consiste à ne considérer que la transition \(\ket{g,\,N} \rightarrow \ket{e,\,N-1}\), 
%c’est-à-dire les processus dans lesquels un seul photon est échangé entre l’atome et le champ laser.

Cette approximation consiste à ne conserver que la transition 
\(\ket{g,\,N} \rightarrow \ket{e,\,N-1}\), c’est-à-dire le couplage 
entre les états \(\ket{g,\,N}\) et \(\ket{e,\,N-1}\), qui sont 
quasi-résonants.%
\footnote{
Dans ce couplage, l’atome passe de l’état électronique 
\(\ket{g}\) à \(\ket{e}\) tandis que le champ voit son nombre de photons 
diminuer de \(N\) à \(N-1\).  
Cela correspond à l’absorption \emph{d’exactement un photon} par l’atome.  
Autrement dit, l’énergie manquante entre \(E_{g,N}\) et \(E_{e,N-1}\) 
provient d’un unique quantum \(\hbar\omega\) du champ.  
Toutes les autres transitions, telles que 
\(\ket{g,\,N} \rightarrow \ket{e,\,N+1}\) (émission stimulée hors résonance) 
ou les couplages multi-photoniques, sont fortement décalées en énergie et 
donc négligées dans l’approximation séculaire.  
La notion « d’un seul photon échangé » signifie donc qu’on ne retient que 
les processus pour lesquels la variation du nombre de photons est 
\(\Delta N = -1\), ce qui est l’ordre dominant dans la limite 
de couplage dipolaire faible et de résonance proche.}
...
L’atome de Rubidium 87 (\textsuperscript{87}Rb) est un \textbf{élément alcalin}. Il possède une configuration électronique de la forme $[ \mathrm{Kr} ]\,5s^1$, avec un unique électron de valence situé dans la couche $5s$. Cela implique que la structure énergétique de l’atome est essentiellement déterminée par ce seul électron périphérique, interagissant avec un cœur atomique fermé (couche interne complète).

En première approximation, on peut donc modéliser le Rubidium comme un système à un électron, à la manière de l’atome d’hydrogène, mais avec un potentiel effectif tenant compte du \emph{blindage} dû aux électrons du cœur \footnote{
Le terme \emph{blindage} (ou \emph{screening}) désigne la réduction du champ électrique 
coulombien du noyau ressentie par l’électron de valence.  
Dans un atome alcalin comme le Rubidium, les électrons du cœur forment une distribution de charge 
qui repousse partiellement l’électron externe et « masque » (écrante) en partie la charge nucléaire.  
Ainsi, l’électron de valence n’interagit pas avec la charge $+Ze$ complète du noyau, 
mais avec une charge effective plus faible $+Z_{\mathrm{eff}}e$.  
Le potentiel central ressenti n’est donc plus strictement coulombien, ce qui explique 
la structure fine particulière des niveaux et leurs écarts par rapport à l’atome d’hydrogène.}
. Ce modèle permet de comprendre les transitions optiques dominantes de l’atome.
...
\end{tikzpicture}
\caption{Structure fine (transitions D1 et D2) de l’atome de Rubidium $^{87}\mathrm{Rb}$.}
...
En effet, un faisceau à \(\sim 770\,\mathrm{nm}\) est \emph{hors résonance} des transitions 
D1 et D2 du Rubidium, mais avec un désaccord suffisamment grand pour que la structure 
hyperfine puisse être négligée dans un premier temps.  
Plus précisément, dans ce régime où 
\(\Delta \gg \Delta_{\mathrm{hfs}}\)\footnote{
Pour le Rubidium, l’écart hyperfin entre les deux multiplets excités est 
\(E_{5P_{3/2}} - E_{5P_{1/2}} \simeq 7~\mathrm{THz}\).  
Si le laser est désaccordé d’une valeur \(\Delta\) bien supérieure à cette séparation, 
les différents sous-niveaux hyperfins contribuent de manière indiscernable à la polarisation 
induite, de sorte que l’on peut les regrouper dans une description à deux raies effectives 
correspondant aux transitions fines D1 et D2.  
Autrement dit, lorsque \(\Delta\gg\Delta_{\mathrm{hfs}}\), l’atome réagit comme si les niveaux 
hyperfins étaient dégénérés.},
la réponse de l’atome est dominée par les seules transitions fines 
D1 (\(5S_{1/2}\rightarrow 5P_{1/2}\)) et D2 (\(5S_{1/2}\rightarrow 5P_{3/2}\)). 
Dans ce régime, le potentiel dipolaire s’écrit comme une somme pondérée des contributions 
de ces deux transitions proches en énergie, 
c’est-à-dire de toutes les transitions dont le désaccord est du même ordre de grandeur que \(\Delta\), 
tandis que les transitions beaucoup plus énergétiques sont fortement suppres­sées et peuvent être ignorées.
...
%Lorsque la base hyperfine est prise en compte — en particulier dans le contexte du piégeage optique spin-dépendant ou vectoriel — le potentiel dipolaire ne se réduit plus à une simple fonction scalaire, mais devient un opérateur agissant dans l’espace des états internes de l’atome. Dans le cas le plus simple où seuls deux états internes sont pertinents (par exemple deux sous-niveaux hyperfins ou Zeeman), le potentiel dipolaire peut être écrit sous la forme générale :
%\begin{eqnarray}
%	U_{\mathrm{dip}} &=& U_{\mathrm{scal}}^{(0)} \cdot \mbox{Id}_2 \;+\; U^{(1)}_{\mathrm{vec}}\,(\operatorvec{J}\cdot\operatorvec{B}_{\rm fict})
%\;+\; U^{(2)}_{\mathrm{tens}}(\operatorvec{J},\operatorvec{u}),\label{eq:dipole-potential-decomposition}
%\end{eqnarray}
...
%Ces contributions peuvent ensuite être projetées sur la base hyperfine $|F,m_F\rangle$ pertinente pour les atomes alcalins. 

\medskip

Dans un scénario expérimental où l’on restreint l’espace interne de l’atome à deux sous-niveaux hyperfins ou Zeeman (par exemple pour définir un pseudo-spin), le potentiel dipolaire peut être représenté par une matrice $2\times 2$ de la forme générale \cite{Le_Kien_2013}:
\begin{eqnarray}
...
\begin{itemize}[label = $\bullet$]
  ...
  \item \textbf{Terme vectoriel (Zeeman optique)} : $U_{\mathrm{vec}}^{(1)}$ agit comme un champ magnétique fictif (optical Zeeman effect) le long de $\operatorvec{B}_{\rm fict}\propto i(\operatorvec{E}^\ast\times \operatorvec{E})$. En effet, il est proportionnelle à l’opérateur $i(\operatorvec{u}^\ast\times\operatorvec{u})\cdot\operatorvec{J}$ se comportant comme $\operatorvec{J}\cdot \operatorvec{B}_{\rm fict}$. Ainsi, la polarisation circulaire du champ ($i[\operatorvec{u}^\ast\times\operatorvec{u}]\neq 0$) donne un décalage dépendant de l’orientation de $\operatorvec{J}$ (analogue à un effet Zeeman), alors que pour une polarisation rectiligne ($\operatorvec{u}$ réel) ce produit vectoriel s’annule et ce terme vectoriel disparaît. On parle souvent de champ fictif parce que, en convention, le terme vectoriel du Hamiltonien d’interaction s’écrit formellement $\mu_B g_J\,(\operatorvec{J}\cdot\operatorvec{B}_{\rm fict})$. Dans notre cas (polarisation rectiligne), ce terme est donc négligeable.\footnote{
Pour un champ électrique harmonique 
\(\operatorvec{E}(\mathbf{r},t)=\operatorvec{E}(\mathbf{r})\,e^{-i\omega t}+{\rm c.c.}\), 
le vecteur complexe \(\operatorvec{E}(\mathbf{r})\) encode l’amplitude et la polarisation.  
Le produit vectoriel complexe 
\(\operatorvec{E}^\ast\times\operatorvec{E}\) mesure précisément la \emph{chiralité} 
ou la \emph{densité de moment cinétique spin} du champ électromagnétique :  
il est nul pour une polarisation linéaire (\(\operatorvec{E}\) réel) et non nul pour une polarisation circulaire ou elliptique, où il pointe dans la direction de propagation.  
La quantité \(i(\operatorvec{E}^\ast\times\operatorvec{E})\) est proportionnelle à la densité de moment angulaire orbital interne du champ et produit un « champ magnétique fictif » ressenti par le moment angulaire atomique via le terme vectoriel du potentiel ac-Stark.  
Une présentation détaillée de ces propriétés, dans le cadre du couplage lumière–atome et du potentiel optique dipolaire, peut être trouvée dans \cite{Le_Kien_2013}.}


  \item[$\times$] \textbf{Terme tensoriel (négligé)} : $U_{\mathrm{tens}}^{(2)}$ introduit une anisotropie du potentiel selon l’orientation du moment angulaire par rapport à la polarisation du champ. Ce terme est proportionnel à une combinaison quadrupolaire des composantes de $\vec{J}$ :
\begin{eqnarray}
...
Cette structure matricielle du potentiel dipolaire joue un rôle central dans la manipulation cohérente des états internes de l’atome, la réalisation de barrières optiques dépendantes du spin, ou encore la mise en œuvre de qubits dans des réseaux d’atomes piégés. Elle permet également d’exploiter des phénomènes comme les transitions \emph{Raman} induites optiquement ou la séparation de spin dans des pièges optiques.
...
\subsection{Régime de désaccord très important}

Dans ce chapitre, nous ne considérons le régime de désaccord très important que dans le but de calculer les éléments de matrice du moment dipolaire $\operatorvec{D}$. En effet, pour un laser fortement désaccordé par rapport aux transitions électroniques ($|\Delta| \gg \Delta_{\rm fs}, \Delta_{\rm hfs}$, où typiquement pour le rubidium $\Delta_{\rm fs} \sim 7\,\mathrm{THz}$ et $\Delta_{\rm hfs} \sim 6.8\,\mathrm{GHz}$), le champ lumineux ne peut plus distinguer les différents niveaux excités liés aux sous-niveaux $J$ ou $F$ (par exemple les niveaux $5P_{1/2}$ et $5P_{3/2}$ pour le rubidium).

\smallskip

Dans ce régime, les niveaux excités peuvent être traités uniquement en termes de moment angulaire orbital $L$ et de sa projection $m_L$, car le moment dipolaire $\operatorvec{D}$ ne dépend pas du spin de l’électron. Les éléments de matrice $\bra{e}\operatorvec{D}\ket{g}$ se calculent donc simplement en utilisant les états orbitaux, ce qui permet de négliger, dans cette approximation, les détails de la structure fine ou hyperfine .
...
Dans cette base, seule la composante $\operator{D}_q$ associée au changement de moment orbital  $m_L \to m_L' = m_L + q$ est non nulle. En effet,
\begin{eqnarray}\label{chap7:dipolaire:eq:selection_rule}
\bra{m_L'} \operatorvec{D} \cdot \operatorvec{u}_q \ket{m_L} \propto \delta_{m_L', m_L + q},
\end{eqnarray}
ce qui montre que l’opérateur dipolaire ne couple que les états dont les nombres quantiques diffèrent exactement de $q$.

\paragraph{Application du théorème de \emph{Wigner-Eckart}.}
...
Le théorème de \emph{Wigner-Eckart}
\footnote{%
Le théorème de \emph{Wigner-Eckart} affirme que tout élément de matrice d’un opérateur tensoriel
irréductible peut se factoriser en un élément réduit, indépendant des projections magnétiques,
et un coefficient de Clebsch--Gordan contenant toute la dépendance angulaire. Autrement dit,
pour un opérateur tensoriel $T^{(k)}_q$, on a
$
\bra{j',m'} T^{(k)}_q \ket{j,m}
= \bra{j'}\!\|T^{(k)}\|\!\ket{j}\,
\braket{j,k; m,q \mid j',m'}.
$
Cette factorisation simplifie considérablement le calcul des couplages dipolaires.}
permet d’évaluer les éléments de matrice de l’opérateur dipolaire entre états de moment angulaire en factorisant la dépendance radiale et angulaire :
\begin{eqnarray}
...
Considérons la transition électronique entre les niveaux $5S$ et $5P$ du rubidium 87, en négligeant la structure fine (\ie, on travaille dans la base des moments angulaires orbitaux $\ket{L , m_L  } $
...
\begin{eqnarray}
 d_{\scriptstyle 5S \leftrightarrow 5P} = \bra{L' = 1, m_L'=0} \operatorvec{D} \cdot \operatorvec{u}_{q=0} \ket{L = 0, m_L = 0} .
\end{eqnarray}
...
L'expression~\eqref{eq:WignerEckart} reste valable pour les éléments de matrice réduits. %~\cite{CCT_QO}. 
Dans le cas du rubidium 87, on peut ainsi déterminer numériquement la valeur de l’élément $\bra{L'}\!\|\operatorvec{D}\|\!\ket{L}$, noté $d_{\scriptstyle 5S \leftrightarrow 5P}$, à partir des données spectroscopiques expérimentales (fréquence de transition, largeur naturelle, etc.) \cite{Steck2002}.
\begin{eqnarray}
...
Le moment angulaire électronique total $J$ résulte du couplage entre le moment angulaire orbital total $L$ et le moment angulaire de spin $S$ des électrons, formellement noté $J = L + S$\footnote{
Ici, $L$ représente le moment cinétique orbital total des électrons, et non une longueur. 
La notation $J = L + S$ est symbolique : il s’agit d’un couplage vectoriel de moments angulaires quantiques, 
et non d’une simple addition de nombres. Pour un électron de valence dans un atome alcalin, 
le spin est fixé à $S = 1/2$, et $L$ correspond à la valeur quantique associée à la configuration orbitale de l’électron externe. 
Cette notation est largement utilisée dans la littérature (cf. par exemple Sakurai, *Modern Quantum Mechanics*, chap. 3).},
Dans le cas des atomes alcalins, on ne considère que l’électron de valence, pour lequel le spin est fixé à $S = 1/2$.


Lorsque le désaccord du faisceau laser devient comparable à la séparation entre les doublets de structure fine, il est nécessaire de prendre en compte le couplage spin-orbite. Celui-ci donne lieu à une levée de dégénérescence des niveaux d’énergie, conduisant à ce que l’on appelle la structure fine de l’atome. Dans ce régime, les états propres du système s’écrivent dans la base couplée $|L, S; J, m_J\rangle$, où $m_J$ est la projection de $J$ sur l’axe de quantification. Ces niveaux sont représentés sur la figure~\ref{fig:structure_fine_rubidium}.
...
\label{fig:structure_fine_rubidium}
...
%Ce décalage est purement scalaire dans le cas d’un champ à polarisation linéaire, car les contributions vectorielles (champ fictif) disparaissent lorsque $\operatorvec{u}$ est réel. De plus, le terme tensoriel est nul dans l’état fondamental $5S_{1/2}$ du rubidium 87, qui à $J = \tfrac{1}{2}$.

Ce décalage est purement scalaire dans le cas d’un champ à polarisation linéaire, 
car les contributions vectorielles (champ fictif) disparaissent lorsque $\operatorvec{u}$ est réel. 
De plus, le terme tensoriel est nul dans l’état fondamental $5S_{1/2}$ du Rubidium 87, 
qui à $J = \tfrac{1}{2}$, pour toute polarisation.
...
\begin{eqnarray}\label{chap7:dipolaire:eq:5S_decomposed}
...
Continuons avec les états excités $\ket{5\,^2P_{1/2},\,m_J'}$, que l’on peut écrire de manière équivalente comme :
\begin{eqnarray}
...
Donc 
\begin{eqnarray}
	\ket{5\,^2P_{1/2},m_J' = \pm\tfrac12} &=& \mp\sqrt{\tfrac{1}{3}}\,\ket{ m_L'=0,\,m_S'=\pm\tfrac12} \;\pm\;\sqrt{\tfrac{2}{3}}\,\ket{m_L'=\pm 1,\,m_S'=\mp\tfrac12} \label{chap7:dipolaire:eq:5P_decomposed_1}   ,\\ 
	\ket{5\,^2P_{3/2},m_J' = \pm\tfrac12} &=& +\sqrt{\tfrac{2}{3}}\,\ket{m_L'=0,\,m_S'=\pm\tfrac12} \;+\;\sqrt{\tfrac{1}{3}}\,\ket{ m_L'=\pm 1,\,m_S'=\mp\tfrac12} \label{chap7:dipolaire:eq:5P_decomposed_2} ,\\
	\ket{5\,^2P_{3/2},m_J' = \pm\tfrac32} &= & +\ket{m_L'=\pm 1 , m_S' = \pm \tfrac12} \label{chap7:dipolaire:eq:5P_decomposed_3} .  
\end{eqnarray}
...
\begin{eqnarray}\label{chap7:dipolaire:eq:d5S5P}
...
\end{eqnarray}
il vient que les éléments de matrice du moment dipolaire entre les états de structure fine s’évaluent comme suit. En utilisant la décomposition des états dans la base découplée (équations~\eqref{chap7:dipolaire:eq:5S_decomposed}, \eqref{chap7:dipolaire:eq:5P_decomposed_1}, \eqref{chap7:dipolaire:eq:5P_decomposed_2}, \eqref{chap7:dipolaire:eq:5P_decomposed_3} ) et les règles de sélection (équation~\eqref{chap7:dipolaire:eq:selection_rule}), on trouve :
%il vient que en utilisant \eqref{chap7:dipolaire:eq:5S_decomposed}, \eqref\label{chap7:dipolaire:eq:5P_decomposed_1} , \eqref\label{chap7:dipolaire:eq:5P_decomposed_2}, \eqref\label{chap7:dipolaire:eq:5P_decomposed_3} , \eqref{chap7:dipolaire:eq:d5S5P} et les règles de sélection \eqref{chap7:dipolaire:eq:selection_rule} :
\begin{eqnarray}
...
En utilisant les résultats précédents, on peut maintenant calculer le potentiel dipolaire ressenti par un atome de rubidium 87 dans l’état fondamental $\ket{5S_{1/2}, m_J = \pm \tfrac12}$, lorsqu’il est soumis à un champ électromagnétique de polarisation linéaire (composante $q = 0$) et de grande fréquence de désaccord par rapport aux transitions D1 et D2.
Le Potentiel dipolaire s'écrit :
...
avec $\Delta_1 = \omega -\omega_{\scriptstyle \tiny 5S_{1/2} \rightarrow 5P_{1/2}} $ et $\Delta_2 = \omega -\omega_{\scriptstyle \tiny 5S_{1/2} \rightarrow 5P_{3/2}} $.
...
Dans le cas d’un champ à polarisation linéaire et pour un laser fortement désaccordé 
(par rapport aux transitions D1 et D2), le {\bf taux d’émission spontanée} 
$\Gamma_{\mathrm{sc}}(\vec{r})$ associé à la diffusion de photons par l’atome de Rubidium 87 
est donné par :

\begin{eqnarray}\label{eq:7.48}
...
\begin{itemize}[label = $\bullet$]
	...
	\item Le {\bf taux de diffusion spontané}  au point $x^\ast$ défini par $U_{dip}(x^\ast) = U_{dip}(0)/2$ doit rester inférieur à une valeur seuil pour limiter l’échauffement et les pertes :
		\begin{equation}\label{eq:7.51}
			\Gamma_{sp}(x^\ast) \leq \Gamma_{sp}^{(max)} = 2\pi \times 1~s^{-1}.	
		\end{equation}
		...
		\begin{equation}\label{eq:7.52}
			\Gamma_{sp}(0) \leq  2\pi \times 2~s^{-1}.	
		\end{equation}
\end{itemize}

\subsubsection*{Figure de mérite : ratio potentiel / diffusion}

Les conditions des équations~\eqref{eq:7.50} et \eqref{eq:7.51} impliquent que, pour un point particulier \(\vec{r}^*\), le rapport entre la profondeur du piège et la diffusion photonique satisfait
\[
\frac{U_{\rm dip}(\vec{r}^*)}{\hbar \Gamma_{\rm sc}(\vec{r}^*)} > \frac{2\,U_{\rm dip}^{(min)}}{\hbar \Gamma_{\rm sc}^{(max)}} .
\]
En utilisant les Eqs.~\eqref{eq.7.47} et \eqref{eq.7.48}, cette condition se réécrit explicitement sous la forme
%\begin{equation}
%\frac{U_{\rm dip}(\vec{r}^*)}{\hbar \Gamma_{\rm sc}(\vec{r}^*)} 
%= \frac{1}{\Gamma} \cdot 
%\frac{\left( \frac{1}{3\Delta_1} + \frac{2}{3\Delta_2} \right)}
%     {\left( \frac{1}{3\Delta_1^2} + \frac{2}{3\Delta_2^2} \right)}
%> \frac{2\,U_{\rm dip}^{(min)}}{\hbar \Gamma_{\rm sc}^{(max)}}.
%\end{equation}
%
%
%Le compromis entre profondeur de piège et diffusion est quantifié par la figure de mérite suivante :
\begin{equation}
\frac{U_{dip}(\vec{r}^*)}{\hbar \Gamma_{sc}(\vec{r}^*)} = \frac{1}{\Gamma} \cdot \frac{ \left( \frac{1}{3\Delta_1} + \frac{2}{3\Delta_2} \right)}{\left( \frac{1}{3\Delta_1^2} + \frac{2}{3\Delta_2^2} \right)} \geq \frac{2 U_{dip}^{(min)}}{\hbar \Gamma_{sc}^{(max)}}
\end{equation}
...
Le faisceau utilisé est gaussien. On rappelle que le \textit{waist} $w_0$ désigne le rayon du faisceau, défini comme la distance radiale $r$ pour laquelle l’intensité transverse chute à $1/e^2$ de sa valeur au centre du faisceau qui est la position où le faisceau est le plus focalisé. Nous consicérons ici des faisceaux avec un waist de l’ordre de $w_0 \sim 1~\mu\text{m}$.  

\medskip

Dans le cadre d’un piégeage avec un laser désaccordé vers le
bleu, les atomes sont confinés dans une région \textbf{centrale de faible intensité}, située entre deux faisceaux focalisés. Le piégeage repose alors sur la formation de \textbf{parois optiques abruptes}, engendrées par les gradients d’intensité des faisceaux. Pour obtenir un gradient suffisamment marqué, il est nécessaire que chaque faisceau possède un \textbf{waist étroit}.
...
Les conditions des équations~\eqref{eq:7.50} et \eqref{eq:7.51} impliquent alors, en utilisant les expressions \eqref{eq:7.47} et \eqref{eq:7.48} pour le potentiel dipolaire et le taux de diffusion de photons, que la puissance laser doit satisfaire
\begin{equation}\label{eq:7.53}
	P_{\rm min}(\lambda) \le P(\lambda) \le P_{\rm max}(\lambda),
\end{equation}
avec
\begin{align}
P_{\rm min}(\lambda) &= U_{\rm dip}^{(min)} \cdot \frac{\pi w_0^2}{2} \frac{8 I_{\rm sat}}{\Gamma^2} 
\left( \frac{1}{3 \Delta_1} + \frac{2}{3 \Delta_2} \right)^{-1}, \\
P_{\rm max}(\lambda) &= \Gamma_{\rm sc}^{(max)} \cdot \pi w_0^2 \frac{8 I_{\rm sat}}{\Gamma^3} 
\left( \frac{1}{3 \Delta_1^2} + \frac{2}{3 \Delta_2^2} \right)^{-1}.
\end{align}

\medskip

La région vérifiant le critère ci-dessus est représentée sur la figure~\eqref{fig:7.6}, dans un diagramme (\(\lambda, P\)) où \(\lambda\) est la longueur d'onde du laser.  

\medskip

Le seuil inférieur \(P_{\rm min}(\lambda)\) est extrêmement faible et n’a pratiquement aucune influence sur le choix expérimental de la puissance laser, contrairement à \(P_{\rm max}(\lambda)\) qui fixe la limite supérieure imposée par la diffusion photonique.
...
Au niveau des atomes, la puissance des lasers doit donc satisfaire \eqref{eq:7.53}, soit être entre la courbe (en bleue) $P^{\rm (min)}(\lambda)$ et (en jaune) $P^{\rm (max)}(\lambda)$ de la figure \eqref{fig:7.6} (et après dans la figure \eqref{chap7:fig:puiss.2}). En tenant compte des pertes optiques, cela se traduit par la condition suivante sur la puissance initiale des lasers. Donc, nous cherchons des lasers dans la zone de tolérances (en vert) de la figure \eqref{fig:7.6} (et après dans la figure \eqref{chap7:fig:puiss.2}). 
...
Trois sources laser ont été identifiées pour la réalisation de la boîte optique blue-detuned destinée au piégeage dipolaire du Rubidium-87, dont deux répondent aux conditions de puissance admissibles $P^{\rm (min)}(\lambda)$ et $P^{\rm (max)}(\lambda)$.
...
Les troix lasers  se situent dans la zone de tolérance définie par les conditions sur la puissance minimale et maximale en fonction de la longueur d’onde, validant leur adéquation pour la construction de notre boîte optique blue-detuned.

\medskip

Avec un waist de $w_0 = 1~\mu\text{m}$, les puissances minimales requises pour satisfaire les conditions $U_{dip}^{(min)}$ et $\Gamma_{sp}^{(max)}$ sont extrêmement faibles : environ $1.1~\mu\text{W}$ pour la \gls{DFB} à $772.5~\text{nm}$, $0.3~\mu\text{W}$ pour la \gls{DBR} à $778.0~\text{nm}$, et $1.4~\mu\text{W}$ pour la DBR à $770.5~\text{nm}$. Ces valeurs sont très largement inférieures aux puissances disponibles pour chacun de ces lasers, même en tenant compte des pertes optiques, ce qui confirme la faisabilité expérimentale du dispositif.
...
Il faut cependant que le modulateur acousto-optique \gls{AOM} puisse modifier l’intensité et la position du faisceau plus rapidement que le temps de réaction du mouvement du centre de masse des atomes. Ce projet est mené par Florence Nogrette.
...
%Avec un waist de $w_0 = 300~\mu\text{m}$, les puissances minimales requises augmentent considérablement en raison de la baisse d’intensité au centre du faisceau. On obtient alors des puissances nécessaires de l’ordre de $102~\text{mW}$ pour la DFB à $772.5~\text{nm}$, $30~\text{mW}$ pour la DBR à $778.0~\text{nm}$, et $127~\text{mW}$ pour la DBR à $770.5~\text{nm}$ (Fig . \ref{chap7:fig:puiss.2} ). Pour assurer un potentiel dipolaire plus propre avec les moins de diffusion et de chauffauge possible, on a choisi la source laser avec le desaccord le plus grand : la DBR à $770.5~\text{nm}$. Neanmoins pour avoir une puit assez profond sa puissance n'est pas assez.  %Seul le laser à $778.0~\text{nm}$ satisfait cette condition sans amplification supplémentaire. En revanche, les deux autres lasers se trouvent sous-dimensionnés dans ce scénario, ce qui justifie le recours à un amplificateur optique dans le cas du laser à $770.5~\text{nm}$.
...
Avec un waist de $w_0 = 300~\mu\text{m}$, les puissances minimales requises augmentent considérablement pour conserver la même intensité au centre du faisceau. On obtient ainsi des puissances nécessaires de l’ordre de $102~\text{mW}$ pour la \gls{DFB} à $772.5~\text{nm}$, $30~\text{mW}$ pour la \gls{DBR} à $778.0~\text{nm}$, et $127~\text{mW}$ pour la \gls{DBR} à $770.5~\text{nm}$ (Fig.~\ref{chap7:fig:puiss.2}).  

\medskip

Afin d’assurer un potentiel dipolaire plus propre, avec le moins de diffusion et de chauffage possible, nous avons choisi la source laser présentant le plus grand désaccord : la \gls{DBR} à $770.5~\text{nm}$. Néanmoins, pour obtenir un puits suffisamment profond, la puissance disponible reste insuffisante.  

\medskip

Ce constat a motivé l’ajout d’un amplificateur optique (\gls{TA} — \emph{Tapered Amplifier}) en sortie du laser à $770.5~\text{nm}$, afin de compenser la puissance limitée et de garantir des conditions de piégeage satisfaisantes malgré les pertes optiques du système.
...
Afin de compenser le déficit de puissance du laser \gls{DBR} à $770.5~\text{nm}$ — notamment dans le scénario initialement envisagé avec un waist focalisé important — un amplificateur optique de type \gls{TA} a été ajouté en aval de la source laser.

\medskip

%Un \gls{TA} est un semi-conducteur optique amplificateur non résonant, composé d'une région d'entrée étroite (guidée) et d'une région de sortie évasée (tapered), permettant d'amplifier efficacement un faisceau monomode tout en conservant un profil spatial de bonne qualité. Contrairement aux lasers, le \gls*{TA} n'oscille pas par lui-même : il agit uniquement sur un signal incident (seed). Il peut amplifier des puissances faibles (quelques mW) en entrée jusqu’à plusieurs centaines de mW, voire au-delà, en sortie.

Un \gls{TA} est un semi-conducteur optique amplificateur non résonant, composé d'une région d'entrée étroite (guidée) et d'une région de sortie évasée (tapered), permettant d'amplifier efficacement un faisceau monomode tout en conservant un profil spatial de bonne qualité. Contrairement aux lasers, le \gls{TA} n'oscille pas par lui-même : il agit uniquement sur une source laser fournie en entrée. Il peut amplifier des puissances faibles, de l'ordre de $20~mW$, jusqu’à plusieurs centaines de mW, voire au-delà, en sortie.
...
%\begin{itemize}[label=$\bullet$]
%    \item \textbf{Sensibilité aux rétro-réflexions :} la puce du TA ne supporte pas les retours optiques parasites, qui peuvent entraîner des instabilités, des dégradations de performance, voire endommager le composant. Un \textbf{isolateur optique} est donc placé en sortie pour bloquer tout retour de lumière vers le \gls{TA}.
%    
%    \item \textbf{Sensibilité à l’alignement :} une injection efficace du faisceau dans la zone guidée d’entrée du \gls{TA} requiert un alignement très précis. Celui-ci est réalisé à l’aide de deux miroirs montés sur des supports à réglage fin, offrant 4 degrés de liberté (translations X–Y, inclinaisons $\theta_x$, $\theta_y$).
%    
%    \item \textbf{Émission multi-modes en sortie :} malgré une injection monomode, la sortie du TA présente parfois plusieurs lobes d’intensité dus à la géométrie évasée. Un trou de filtrage ou une \textbf{trou de filtrage} de $50~\mu$m (à ajuster selon le profil observé) est donc utilisé pour filtrer spatialement le mode fondamental TEM$_{00}$.
%\end{itemize}

\begin{itemize}[label=$\bullet$]
    \item \textbf{Sensibilité aux rétro-réflexions :} la puce du \gls{TA} ne supporte pas les retours optiques parasites, qui peuvent entraîner des instabilités, des dégradations de performance, voire endommager le composant. Un \textbf{isolateur optique} est donc placé en sortie pour bloquer tout retour de lumière vers le \gls{TA}.
    
    \item \textbf{Mode spatial en sortie :} la sortie du \gls{TA} reste monomode, mais le profil spatial est fortement déformé et s’éloigne d’un mode TEM$_{00}$ parfait. Un  \textbf{trou de filtrage} de $50~\mu$m (à ajuster selon le profil observé) peut être utilisé pour filtrer et améliorer la qualité spatiale du faisceau.
\end{itemize}
...
\begin{itemize}[label=$\triangleright$]
    \item \textbf{Laser utilisé pour injecter le TA :} le laser DBR à $770.5~\text{nm}$ est stabilisé en température et en courant à l’aide du contrôleur Thorlabs ITC-510, et injecté dans le \gls*{TA} à l’aide d’une optique de couplage sur table optique.
    ...
    \item \textbf{Filtrage spatial :} un trou de filtrage (trou de filtrage) placé dans le plan de Fourier d’un système en configuration télescopique, positionné en sortie de l’amplificateur \gls{TA}, permet de sélectionner efficacement le mode spatial fondamental.

    \item \textbf{Isolateur optique :} un isolateur optique est placé immédiatement après le \gls{TA} pour éviter tout retour de lumière vers la puce.

    \item \textbf{Modulation de puissance :} un modulateur acousto-optique \gls{AOM} est inséré juste avant une fibre optique, permettant de couper la lumière à la demande (switch rapide) ou de moduler dynamiquement l’intensité laser. Cela permet un contrôle fin de l’illumination au niveau des atomes.
    
    \item \textbf{Mesure de puissance :} une photodiode ou un puissance-mètre placé après l’\gls{AOM} permet de mesurer précisément la puissance en fonction du courant injecté dans le \gls{TA}, et de corriger les effets d’instabilité si nécessaire.

\end{itemize}

\subsubsection*{Caractérisation et performance}

La diode laser et le \gls{TA} sont asservis à une température stable afin d’éviter toute fluctuation susceptible d’affecter leur fonctionnement. Une courbe de puissance de sortie du \gls{TA} en fonction du courant injecté a été mesurée (voir figure~\ref{fig:Puissance_TA}), avec la diode laser fournissant une puissance maximale d’environ $31.2\,\text{mW}$ et le TA testé jusqu’à un courant d’injection de $2.5\,\text{A}$.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{figures/07_Dipolaire/Puissance_TA.png}
    \caption{Puissance de sortie du \gls*{TA} en fonction du courant d’injection, pour une puissance d’entrée provenant de la diode laser fixée à 25 mW.}
    \label{fig:Puissance_TA}
\end{figure}

Afin d’éviter une dégradation prématurée des composants, il a été choisi de ne pas les faire fonctionner à leurs limites maximales. La diode laser est ainsi réglée pour fournir une puissance d’environ $25\,\text{mW}$ (courant $I_{\text{diode}} = 150.6\,\text{mA}$) et le \gls{TA} est exploité avec un courant d’environ $1.5\,\text{A}$. Cette configuration assure un bon compromis entre puissance de sortie et longévité des dispositifs, tout en garantissant une amplification stable et un profil spatial préservé.


\medskip

Pour un courant d’injection de $1.5,\text{A}$ dans le \gls{TA}, la puissance mesurée en sortie, après passage dans l'\gls{AOM}, est d’environ $62,\text{mW}$.
Or, d’après la figure \ref{fig:TA_setup}, à $1.5,\text{A}$, la puissance en sortie directe du \gls{TA} atteint environ $750,\text{mW}$. Cela signifie qu’après l’\gls{AOM}, il ne reste qu’environ $8\%$ de la puissance initiale, ce qui n’est pas raisonnable.

\smallskip

Cette perte importante s’explique par le fait que l’on ne souhaite sélectionner que l’ordre fondamental gaussien du faisceau, alors que le \gls{TA} émet une superposition de modes. Une fraction trop importante de la puissance est ainsi distribuée dans les modes d’ordre supérieur, qui sont rejetés.

\smallskip

De plus, le \gls{TA} installé présente un rendement maximal (proche de $100\%$ de la puissance spécifiée) lorsque le laser d’entrée est à $780,\text{nm}$, alors qu’à $770.5,\text{nm}$ le rendement chute à environ $45\%$. Dans notre cas, nous avons utilisé un \gls{TA} déjà disponible au laboratoire, choisi comme le plus adapté parmi ceux dont nous disposions, afin d’éviter l’achat d’un modèle plus spécifique.

\smallskip

Il semble donc nécessaire d’envisager, à terme, le remplacement par un \gls{TA} mieux adapté à la longueur d’onde utilisée.

\medskip

L’\gls{AOM} génère plusieurs ordres de diffraction : le mode zéro (non diffracté), les modes $\pm 1$, etc. Afin d’optimiser le transfert d’énergie dans un mode diffracté souhaité (par exemple le premier ordre $\pm 1$), le cristal \gls{AOM} a été ajusté finement en rotation. Ce réglage permet de maximiser la puissance dans ce mode non nul.
...
Ainsi, l’\gls{AOM} est configuré de manière à transférer efficacement la puissance vers le mode diffracté utile, avec une puissance d’environ $62\,\text{mW}$ disponible à l'entée de la fibre, garantissant une qualité spatiale et une polarisation stables pour l’expérience.
...
Dans ce chapitre, nous avons présenté les dispositifs expérimentaux permettant de réaliser et de contrôler des gaz atomiques confinés en une dimension. Le potentiel dipolaire offert par la puce atomique permet déjà de créer des profils de densité variés, jusqu'à un potentiel quartique en ordre 4. 
L’introduction du \gls{DMD} offre une flexibilité supplémentaire : il permet de sélectionner des régions du nuage atomique et de générer des découpes locales, ouvrant ainsi la voie à des préparations plus complexes.

\medskip

En combinant le potentiel de la puce et le \gls{DMD}, il est possible de préparer des nuages atomiques homogènes sur des zones plus étendues que celles limitées par le potentiel quartique de la puce seule. Cependant, pour générer des potentiels encore plus exotiques et modulables, l’utilisation d’un \gls{TA} est nécessaire. Après optimisation, nous avons injecté environ 62~mW dans la fibre conduisant vers le \gls{DMD}. Cette configuration est satisfaisante pour le projet en cours, mais elle révèle certaines limitations du \gls{TA} utilisé : le profil spatial du faisceau s’écarte significativement d’un mode TEM$_{00}$, ce qui explique la faible puissance réellement injectée. Pour atteindre des performances supérieures et exploiter pleinement la flexibilité du \gls{DMD}, il serait nécessaire de remplacer le \gls{TA} actuel par un modèle offrant un profil spatial plus adapté.
