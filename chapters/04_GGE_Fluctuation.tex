...
\begin{eqnarray}\label{chap4:eq.charge.q.1}
	\operator{\mathcal{Q}}[f] = L\int_0^L d\theta f(\theta) \operator{\rho}(\theta),\quad \text{où $\operator{\rho}(\theta)$ agit comme } \quad \operator{\rho}(\theta) \ket{ \{ \theta_a \} } = \frac{1}{L} \sum \delta ( \theta - \theta_a ) \ket{ \{ \theta_a \} }
\end{eqnarray}
...
Comme introduit dans l'équation \eqref{chap4:eq.diff.q.1},  la {\bf dérivée fonctionnelle dans la direction d’une fonction test} $f$ appliquée à un fonctionnel $F[g]$, comme :
...
\begin{equation}
	\chi_w(\theta, \theta') \doteq -\frac{1}{L} \frac{\delta \langle \operator{\rho}(\theta) \rangle}{\delta \omega(\theta')} 
\end{equation}
...
À l’équilibre thermodynamique, cette distribution satisfait l’équation \eqref{chap.2:eq.rho.eq.1}. Elle coïncide avec la moyenne de l’opérateur de densité de rapidité, soit : 
\begin{eqnarray}
	 \rho_{\mathrm{eq}} =  \braket{ \operator{\rho}(\theta) }_w	
\end{eqnarray}
ce qui représente la densité continue de quasi-particules dans l’état stationnaire d’équilibre.

Dans le cadre du GGE continu, l’état macroscopique est entièrement déterminé par une densité spectrale $\rho(\theta)$ qui maximise l’entropie de Yang-Yang $\mathcal{S}_{\mathrm{YY}}[\rho]$, sous la contrainte d’une charge généralisée fixée. Le poids spectral $w$ est fixé. On a vus dans les chapitre (\ref{chap:relaxation}) que la condition d’équilibre \eqref{chap:TBA:eq:stationnarite-2} s’écrit :
\begin{eqnarray}
...
\begin{eqnarray}
	\chi_w(\theta, \theta') = \left( L \mathcal{H}^{(\mathcal{S}_{\mathrm{YY}})} \right)^{-1} (\theta, \theta').
\end{eqnarray}
