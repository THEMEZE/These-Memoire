...
On souhaite tester notre capaciter à eéchantillonner à échantillonner le \gls{GGE}avec Metropolice. Pour cela on va utiliser le principe de \emph{fluctuation-réponse} définie ci dessus dans l'équation \eqref{chap4:eq.susceptibilite-fonctionnelle}. On va dans un premier temps calculer numériquement les corrélation des distrivution de rapidité en utilisant un algorithme de Monte Carlo basé sur les états propres du modèle de \gls{LL} et dans un deuxiemment temps en utilisant la suceptibilité.
...
\end{eqnarray}
où \( \{I_a\} \) sont des entiers (ou demi-entiers) représentant une configuration de type Bethe.

\medskip

\subparagraph{Initialisation : état fondamental.}
\begin{enumerate}
    \item On commence par proposer une configuration d'entier de Bethe  \( \{I_a\} \) correspondant à l’état fondamental. Celle-ci est donnée par l'équation ~\eqref{chap:1:eq:EBA.1} que l'on rappelle ,
		\begin{eqnarray}
			I_a & = & a - \frac{N+1}{2}, \quad a \in \llbracket 1 , N \rrbracket	
		\end{eqnarray}

    \item On résout ensuite les équations de Bethe associées afin d'obtenir l’ensemble des rapidités \( \{ \theta_a \} \).
...
Pour \(N_{\text{conf}} = 2*10^6\), nous avons répété dix fois l’algorithme de Monte Carlo afin de calculer l’écart-type des moyennes \(\mathrm{std}(\langle \rho^e(\theta) \rangle)\). 
...
Dans notre cas, cette quantité est inférieure à \(2\%\), ce qui montre que \(\langle \rho^e(\theta) \rangle\) constitue une très bonne approximation de la moyenne \(\braket{\operator{\rho}(\theta)}_w\). Ainsi, pour \( N_{\text{conf}} = 2*10^6 \), les équations \eqref{cahp4:eq.rho.e.1} et \eqref{cahp4:eq.corre.e.1} sont bien vérifiées. 
...
Nous avons généré \( N_{\text{conf}} = 2*10^6 \) configurations et calculé les susceptibilités avec un paramètre de perturbation \( \varepsilon = 10^{-1} \). Et \eqref{cahp4:eq.sus.e.1} était verier, la matrice obtenue sera notée $\chi_w(\theta, \theta')$ (Fig \ref{fig:comparison_chi_C}).
...
- Le calcul de \(C_w(\theta, \theta')\) est en \(\mathcal{O}(N_{\rm conf})\) et celui de \(\chi_w(\theta, \theta')\) en \(\mathcal{O}(N_{\rm conf} \cdot N_{\rm cell})\), où \(N_{\rm cell}\) est le nombre de points de la grille en \(\theta\).  
- Pour \(N_{\rm conf} = 2\times 10^6\) et \(N_{\rm cell} = 20\), la matrice \(C_w(\theta, \theta')\) se calcule en environ 6 minutes et \(\chi_w(\theta, \theta')\) en environ 2 heures.  
- On peut envisager \(N_{\rm conf} = 10^7\) : \(C_w(\theta, \theta')\) en 30 minutes et \(\chi_w(\theta, \theta')\) en 10 heures.
...
Sous l’approximation gaussienne autour de l’équilibre (en particulier lorsque \(L \to \infty\)), la covariance des fluctuations est donnée par :
\begin{eqnarray}
...
À partir de ces paramètres, on résout numériquement les {\em équations } \gls{TBA} \eqref{chap:TBA:eq:e}, ce qui permet d'obtenir les grandeurs thermodynamiques d'équilibre :
\begin{itemize}[label = $\bullet$]
...
À partir de ces paramètres, et suivant la même procédure numérique que dans la sous-section précédente, on obtient la distribution de rapidité à l'équilibre \( \braket{\operator{\rho}(\theta)}_w \equiv \rho_{\mathrm{eq}}(\theta) \), ainsi que les composantes de l’inverse de l’opérateur hessien \( (L\mathcal{H})^{-1} \) : la contribution diagonale sans interaction \( (L\mathcal{D})^{-1} \) et la contribution non locale issue des interactions \( \mathcal{B}/L \).
...
%\]
\begin{eqnarray*}
	\frac{\mathcal{D}^{-1}(\theta, \theta')}{\delta(\theta - \theta')} 	
\end{eqnarray*}
