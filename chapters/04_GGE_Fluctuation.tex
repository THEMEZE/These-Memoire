...
\begin{enumerate}
	...
	\item Enfin, on enregistre la distribution de rapidité empirique associée à la configuration \( \{\theta_a\} \), en la discrétisant sur une grille fixée \( \{\theta_a\} \).

\end{enumerate}

\medskip

Un grand nombre d’itérations est réalisé, conduisant à la génération de 
\( N_{\text{conf}} \) configurations (non nécessairement distinctes), 
permettant d’assurer une bonne convergence statistique des moyennes.
...
\begin{itemize}[label = $\bullet$]
    ...
    \braket{\delta \operator{\rho}(\theta) \, \delta \operator{\rho}(\theta')}_w,
    \)
    obtenue en calculant, sur l’ensemble des configurations Monte Carlo, la moyenne du produit des fluctuations locales aux points \(\theta\) et \(\theta'\) :
    \begin{eqnarray*}
    	C_w(\theta, \theta') 
        = \frac{1}{N_{\text{conf}}} \sum_{k=1}^{N_{\text{conf}}} 
        \delta \operator{\rho}_k(\theta) \, \delta \operator{\rho}_k(\theta').	
    \end{eqnarray*}
    Autrement dit, il s’agit de la covariance empirique des profils générés.
\end{itemize}
...
Nous avons généré \( N_{\text{conf}} = 10^7 \) configurations et calculé les susceptibilités avec un paramètre de perturbation \( \varepsilon = 10^{-2} \).
...
\begin{figure}[H]
    ...
    \caption{Comparaison des matrices : à gauche , la corrélation  \( C_w(\theta,\theta') \) ; à droite, la susceptibilité \( \chi_w(\theta,\theta') \) obtenue par dérivée fonctionnelle} 
