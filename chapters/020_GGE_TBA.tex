...
Avant relaxation, un sous-système est décrit par une infinité de données initiales, par exemple les positions et les impulsions microscopiques $\{ \operator{x}_i , \operator{p}_i \}$ de toutes les particules.
...
À l’inverse, dans les sous-systèmes {\bf chaotiques}, ce qui émerge après relaxation est l’équilibre thermique standard. Dans ce cas, la dynamique chaotique “efface” presque toute l’information initiale : l’infinité de données microscopiques avant relaxation converge vers un nombre fini de quantités pertinentes, typiquement l’énergie (et éventuellement le nombre de particules). Le modèle statistique approprié n’est alors pas le \gls{GGE}, mais simplement le modèle de Gibbs (l'Ensemble de Gibbs \gls{GE}), caractérisé par un petit nombre de multiplicateurs de Lagrange : la température (et éventuellement le potentiel chimique).
...
\begin{itemize}[label=$\bullet$]
	...
	\item Si la charge conservée est $\operator{Q}_2^{(\mathcal{S})}-\mu\operator{Q}_0^{(\mathcal{S})}  = \operator{H}^{(\mathcal{S})} - \mu \operator{Q}^{(\mathcal{S})} $ (ensemble grand canonique), alors le multiplicateur est simplement $ \beta$.
\end{itemize}

Dans ce cas particulier, la matrice densité locale \eqref{chap.TBA.op.rho.S} se réduit à celle de l’{\bf ensemble grand canonique de Gibbs} :
\begin{eqnarray}
...
Les moyennes et les fluctuations thermodynamiques usuelles s’expriment naturellement comme dérivées du logarithme de la fonction de partition $\Xi^{(\mathcal{S})}_{\mathrm{GE}}$ :
\begin{eqnarray}
...
\end{eqnarray}
où $ L^{(\mathcal{S})}$ est la taille du support $\mathcal{S}$ et  $\operator{\rho}^{(\mathcal{S})}(\theta)$ agit sur une état de Bethe comme 
...
Dans la base des états de Bethe \( \{ \ket{\{ \theta_a \}} \} \), l’opérateur \( \hat{\rho}(\theta) \) défini en \eqref{chap.2.rho.1} est diagonal, et ses valeurs propres correspondent aux {\bf distributions de rapidité} associées à chaque état de Bethe.
...
$\Omega(\theta)$ est jamais null donc on peut approximer, pour de grandes valeurs de $L$ : 
\begin{eqnarray}
...
L’exposant $e^{L(\mathcal{S}_{YY}-\mathcal{W})[\rho]}$ fixe la \textbf{probabilité relative} d’un macro-état $\rho(\theta)$ dans le GGE : le terme entropique favorise les états, tandis que le terme énergétique pénalise les configurations coûteuses — d’où la compétition caractéristique del’équilibre statistique.
