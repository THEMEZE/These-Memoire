...
Dans le chapitre précédent (\ref{chap:LL-BA}), on a vu que l'état $\ket{\{ \theta_a \}}$ associé à cette configuration est un état propre des observables nombre d'atome, quantité de mouvement  et  énergie totale \eqref{chap1:eq.Q.P.K.theta.1}. Ces observables sont diagonales dans la base des états propres :
\begin{eqnarray}
...
Dans un système quantique {\bf intégrable}, il existe une infinité de charges conservées locales $\operator{Q}_i$ commutant entre elles et avec l’Hamiltonien $\operator{H}$  \cite{rigol2007relaxation, rigol2007}. Concrètement, chaque charge se présente sous la forme $\operator{Q}_i = \int dx \,\operator{q}_i(x)$, où $\operator{q}_i(x)$ est une densité d’observable locale à support borné \footnote{Dans le chapitre suivant \ref{chap:GHD}, nous développerons davantage $\operator{q}_i(x)$ à l’échelle mésoscopique et en hydrodynamique. Dans ce chapitre, nous nous limitons à la thermodynamique, mais je le mentionne ici pour introduire le formalisme.} . L’intégrabilité implique une caractérisation complète des états propres par un ensemble de paramètres (rapidités $\{\theta_j\}$ dans le modèle de \gls{LL}) \cite{Nardis_2016}. En particulier, contrairement aux systèmes génériques, un système intégrable ne thermalise pas au sens canonique classique, car la présence de toutes ces contraintes empêche l’oubli des conditions initiales. Les points clés sont alors :
...
Pour décrire cet état, on introduit le \gls{GGE} \cite{rigol2007relaxation}, ont montré qu’une « extension naturelle de l’ensemble de Gibbs aux systèmes intégrables » prédit correctement les valeurs moyennes des observables après relaxation.  
...
\end{eqnarray}
de la charge locale $\operator{Q}_i^{(\mathcal{S})}$, en injectant \eqref{chap.TBA.moy.2} et \eqref{chap.TBA.mom.1} et en utilisant $\frac{1}{f} \partial_x^2 f - ( \frac{1}{f} \partial_x f ) = \partial_x^2 \ln f $ :
\begin{eqnarray}
...
Dans le paragraphe précédent, nous avons sous-entendu — sans l’expliciter — qu’il est montré que l’ensemble des charges locales conservées forme une famille donnée par :
\begin{eqnarray}
...
\)
on associe un opérateur-charge local :
...
\(
  f_2(\theta)=\theta^{2}/2
\)
redonnent respectivement l'opérateur nombre \(\operator{Q}=\operator{Q}_0^{(\mathcal{S})} = \operator{\mathcal{Q}}^{(\mathcal{S})}[1]\) , impulsion \(\operator{P}=\operator{Q}_1^{(\mathcal{S})} = \operator{\mathcal{Q}}^{(\mathcal{S})}[\theta]\) et énergie
\(\operator{H}=\operator{Q}_2^{(\mathcal{S})} = \operator{\mathcal{Q}}^{(\mathcal{S})}[\theta^2/2]\). Et dans le cadre des (\gls{GGE}), pour tous les ordres $i$ on note :
...
Dans le cas des modèles intégrables traités par le Bethe Ansatz, il est toutefois plus pratique de réécrire cette matrice densité  $\operator{\rho}^{(\mathcal{S})}_{\mathrm{GGE}}$ de \eqref{chap.TBA.op.rho.S} en termes du \emph{poids spectral} $w(\theta)$ introduit en \eqref{chap.2.w.1}. On obtient alors :
...
\end{eqnarray}	
et la fonction de partition \eqref{chap.TBA.op.Z.S} s'écrit $Z^{(\mathcal{S})}[w]\doteq \bm{\mathrm{Tr}}\left [ e^{-\operator{\mathcal{Q}}^{(\mathcal{S})}[w]}\right ] $ et vaux :
\begin{eqnarray}
...
Nous verrons comment, dans la limite thermodynamique, les sommes sur les configurations de rapidités se transforment en intégrales sur des fonctions continues, comment apparaît l’entropie de Yang–Yang, et comment les moyennes de l’ensemble généralisé se réexpriment à l’aide de ces densités macroscopiques.
...
\end{eqnarray}
où $\sum_{\rho }$ est une somme sur tous les distributions de rapidité $\rho$ et 
...
\medskip
Pour établir la fonction $\Omega[\rho]$, rappelons-nous de la transformation des équations de Bethe dans la limite thermodynamique, hors état fondamental \eqref{eq:TBA-nu} et \eqref{eq:TBA-rhos-2}.
\begin{equation}
...
Cette formalisation constitue la brique de base de l'\textbf{hydrodynamique généralisée} et, dans la section suivante, permet de définir rigoureusement l’\textbf{entropie de Yang–Yang}, indispensable pour décrire la relaxation hors d’équilibre des systèmes intégrables.
...
Prenons un {\em poids spectral} quelconque, par exemple : 
\begin{equation}
...
\begin{itemize}[label = $\bullet$]
    \item \textbf{Régime de Tonks-Girardeau} : ce régime correspond à des interactions fortes, $\gamma \to \infty$ (ou $g \to \infty$, densité faible). Dans ce cas, la distribution de rapidités ressemble à une \emph{mer de Fermi}. %Un exemple de ce profil est représenté sur la Fig.~\ref{fig:rapidites}(a).
...
\begin{figure}[h!]
    ...
    \caption{Distributions de rapidités pour différents paramètres de Lieb $\gamma$ et différentes températures $T$.  Distribution dans le régime de faible interaction ($\gamma \ll 1$) : le demi-cercle gris représente la distribution attendue pour $\gamma \to 0$, tandis que le profil bleu correspond à $\gamma$ petit mais non nul, montrant une discontinuité aux extrémités. La distribution jaune illustre l’état excité au-delà de l’état fondamental, avec apparition d’ailes aux extrémités.}
    \label{fig:rapidites}
\end{figure}
...
\begin{figure}[h!]
    ...
    \caption{Diagramme des régimes principaux du gaz de \acrlong{LL} : gaz de Bose idéal, Tonks-Girardeau et \gls{qBEC}, en fonction des paramètres adimensionnés $\gamma$ et $\mathfrak{t}$. Les courbes noires, bleues et orange délimitent les cross-overs entre régimes. La zone grise correspond aux paramètres expérimentaux explorés pendant la thèse. Figure adaptée de \cite{Bouchoule2011}.}
    \label{fig:diagramme}
\end{figure}

\section*{Conclusion du chapitre}

Dans les systèmes intégrables, l’état stationnaire hors équilibre est correctement décrit par un \gls{GGE} basé sur une infinité de charges conservées. Ces charges sont liées aux puissances des rapidités des quasi-particules.  

\medskip

La description via le \emph{poids spectral} $w(\theta)$ permet de relier ces charges aux distributions de rapidité à l’équilibre. En moyenne, $w(\theta)$ définit à la fois les valeurs moyennes des charges conservées et la distribution de rapidité correspondante. 

\medskip 

Dans la limite thermodynamique, $w(\theta)$ sert de donnée d’entrée pour les équations de le \gls{TBA}, qui permettent, par un calcul numérique, de reconstruire la distribution de rapidité à partir de $w(\theta)$.  
