...
\subparagraph{Quantité de mouvement totale et Nombre totale de particule. }
...
On s'avance sur le chapitre (\ref{chap:relaxation}), en voulant généraliser avec $\operator{f} = \operator{p}^q$ où $q$ est un entier. Soit dans la base \(\{\ket{k}\}\) : 
\begin{eqnarray}
	\operator{F} = (-i\hbar)^q \sum_k k^q \operator{b}_k^\dagger \operator{b}_k^{} , \quad \operator{F} \ket{\{k_a\}} = (-i\hbar)^q \left( \sum_{i = 1}^N k \right) \ket{\{k_a\}}
\end{eqnarray}
...
\end{eqnarray}
et en appliquant \(\ket{\{k_a\}}\) : 
\begin{eqnarray}
	\operator{F}\ket{ \{k_a\}} =  \int_0^L d^N z \, \operator{\mathcal{F}}_N \, \varphi_{\{k_a\}}(\{z_a\} ) \operator{\Psi}(z_1) \cdots \operator{\Psi}^\dag(z_N) \ket{\emptyset}\, , \quad \mbox{avec} \quad \operator{\mathcal{F}}_N = (-i\hbar)^q \sum_{i=1}^N (\operator{\partial}_{z_i})^q.
\end{eqnarray}
\end{mdframed}
