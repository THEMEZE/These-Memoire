...
En présence d’un Hamiltonien avec interaction (par exemple dans le modèle de Lieb–Liniger Sec \ref{chap1:sssec:LL}), les états propres ne sont plus des produits d’ondes planes $\ket{\{k_a\}}$, et l’opérateur énergie cinétique $\operator{K}$ n’est plus diagonalisé dans cette base. Ce sont alors les états de Bethe qui diagonalise l’Hamiltonien complet (cinétique + interaction).
...
\subsubsection{Expression de l’Hamiltonien de Lieb-Liniger \gls{LL}. }\label{chap1:sssec:LL}
...
\end{eqnarray}
on obtient alors l’équation de \textbf{Schrödinger non linéaire (NS)} (ou équation de Gross–Pitaevskii) pour une fonction d’onde classique :
\begin{eqnarray}
...
\begin{mdframed}[
	...
	Nous verrons au chapitre~(\ref{chap:relaxation}) que, dans les systèmes intégrables comme le Lieb--Liniger, cette situation s’étend à une {\bf \em infinité d’intégrales du mouvement}. 
	Cette richesse supplémentaire de conservation conduira naturellement à l’introduction de l’ensemble de Gibbs généralisé (GGE).
\end{mdframed}
