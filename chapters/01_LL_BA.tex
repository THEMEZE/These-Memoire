...
\subparagraph{Exemple : particule libre en une boite à une dimension.} Pour une particule libre de masse $m$ en une dimension, l’hamiltonien est purement cinétique : $\operator{\mathcal{H}} = \hat{p}^2 / 2m$ soit :
	\begin{eqnarray}
		\operator{\mathcal{H}} = -\frac{\hbar^2}{2m} \partial_x^2.
	\end{eqnarray}
	Les états propres stationnaires dépendant du temps sont de la forme $\psi_k(x,t) = \varphi_k(x)\,e^{-i\varepsilon(k)t/\hbar}$ soit :
	\begin{eqnarray}
	...
	\begin{eqnarray}
		kL \in 2\pi\mathbb{Z}.
	\end{eqnarray}
	Les solutions générales de l’équation de Schrödinger s’écrivent alors comme une superposition d’états propres  $\psi = \sum_k c_k \psi_k $.  Le problème est équivalent à celui d’une particule libre sur un cercle de périmètre $L$.
