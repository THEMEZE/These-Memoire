...
\begin{scope}
	...
	\draw[shift={(0,0)}, color=\colorOne]
		(0, -1.0 ) edge [thick,line width=0.8ex , ->,>=triangle 45  ]node [pos=0.9,left=0.2cm ]{\huge$L(\mathcal{W}-\mathcal{S}_{YY})[\rho]$}( 0  , 5 )
	;
	\draw[]
		(2.5, 0.12 ) edge [thick,line width=0.8ex ,\colorThree ]node [pos=1,below  ]{\huge$\rho_{\mathrm{eq}}$} (2.5, -0.12 )	
	;
	...
	\draw[]
		...
		(-0.3,1) edge [thick,line width=0.4ex  , \colorThree ] node [pos=0,left ]{\huge$L(\mathcal{W}-\mathcal{S}_{YY})[\rho_{\mathrm{eq}}]$} (0.3, 1 )	
	;
