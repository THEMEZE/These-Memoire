...
\addplot+[smooth, \colorFour , thick] table {../02_GGE_TBA/nu_vs_theta.dat};
\end{axis}
...
% rho_s
\addplot+[smooth,\colorSix, thick] table {../02_GGE_TBA/rho_s_vs_theta.dat};
\addlegendentry{$\rho_s(\theta)$}

% rho
\addplot+[smooth,\colorFour, thick] table {../02_GGE_TBA/rho_vs_theta.dat};
...
\begin{tikzpicture}
    ...
    \draw[->, line width=0.8ex, color=\colorOne] 
        (0,0) -- (9,0) 
        node[pos=0.5, below] {\fontsize{16pt}{19pt}\selectfont $\gamma$};

    % Axe vertical (t)
    \draw[->, line width=0.8ex, color=\colorOne] 
        (0,0) -- (0,6) 
        node[pos=0.5, below, rotate=90] {\fontsize{16pt}{19pt}\selectfont $t$};
        
    % Rectangle avec texte "Tonks Girardeau"
    \node[draw, fill=\colorFive, minimum width=3cm, minimum height=2cm, align=center] 
        at (5,4) {\fontsize{1pt}{16pt}\selectfont Tonks Girardeau};
